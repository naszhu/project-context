#= ===========================================================================================================
   ...
   Notes       :  modified from the saved version and try how one stage works
   =========================================================================================================== =#
...
is_onlyaddtrace = false; #add but not strengtening trace
  n_units_time = 12; #number of steps
...
is_onlyprobe_changecontext = false
w_word = 35; # number of word features
b_final = 0.35 #proportion of context features(common)
b_initial = 0.62 #proportion of context features(common)
...
is_store_mismatch = true; #if mismatched value is restored during test
n_finalprobs = 420;
...
w_context =60; #2*15 number of context features
words_per_list = 20;
g_word = 0.3; #geometric base rate
g_context = 0.4; #geometric base rate of context
p_change = 0.1;
...
u_star_context = u_star +0.01 # ratio of this and the next is key for T_nt > T_t, when that for storage and test is seperatly added, also influence
c_context = c +0.20

context_tau_f = 20;

#the following changed magnitude of final test accuracy
p_listchange_finalstudy = 1; #either of these contribute to differences between list 
p_listchange_finaltest = 1;
...
function calculate_two_step_likelihoods(probe::EpisodicImage, image_pool::Vector{EpisodicImage}, p::Float64)::Tuple{Vector{Float64}, Vector{Float64}} 
    ...
    for ii in eachindex(image_pool) 
        ...
        if is_onlyprobe_changecontext  #here is second stage would be wrong
            context_likelihood = calculate_likelihood_ratio(probe_context[16:30],image_context[16:30],g_context,c )  # .#  Context calculation
        else 
...
function calculate_one_step_likelihoods(probe::EpisodicImage, image_pool::Vector{EpisodicImage}, is_finaltest::Bool)::Vector{Float64}
    ...
    probe_features = vcat(probe.word.word_features,probe.context_features[31:60]);
    
    for ii in eachindex(image_pool) 
        image = image_pool[ii];
        image_features = vcat(image.word.word_features,image.context_features[31:60]);
...
function probe_evaluation(image_pool::Vector{EpisodicImage}, probes::Vector{Probe})::Array{Any}
    ...
    for i in eachindex(probes)
        ...
        likelihood_ratios = calculate_one_step_likelihoods(probes[i].image, image_pool, false) #proportion is all
        # likelihood_ratios = likelihood_ratios |> x -> filter(e -> e != 344523466743, x);
