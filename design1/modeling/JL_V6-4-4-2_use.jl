...
criterion_final = LinRange(0.14, 0.18, 10)
final_gap_change = 0.19; #0.21
...
# function EpisodicImage(word::Word,context_features::Vector{Int64},list_number::Int64, final_testpos_img::Int64=0)
#     return EpisodicImage(word, context_features, list_number, final_testpos_img)
# end
# Functions
...
function probe_evaluation2(image_pool::Vector{EpisodicImage}, probes::Vector{Probe})::Array{Any}
    ...
    for i in eachindex(probes)
        ...
        results[i] = (decision_isold=decision_isold, is_target=string(probes[i].classification), odds=odds, list_num=probes[i].image.list_number, rt=rt, initial_studypos=probes[i].image.word.studypos, initial_testpos = probes[i].initial_testpos) #! made changes to results, format different than that in inital
...
function restore_intest(image_pool::Vector{EpisodicImage}, iprobe_img::EpisodicImage, decision_isold::Int64, imax::Int64, probetype::Symbol, list_change_features::Vector{Int64}, general_context_features::Vector{Int64}, odds::Float64, likelihood_ratios::Vector{Float64}, simu_i::Int64, initial_testpos::Int64)
    ...
    # if initial_testpos==1
    #     global counts = 1;
    # elseif initial_testpos == 20
...
if is_finaltest
    ...
    # pf3
    grid.arrange(pf3,pf1,ncol = 1,nrow=2)
    """
end
