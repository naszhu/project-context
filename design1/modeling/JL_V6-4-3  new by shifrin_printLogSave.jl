    ...
    *4. change probe_evluation function: make image_pool_currentlist image pool of each list ininitial only contain current list images. is_onlytest_current_list=false;
    ...
    # p_listchange_finaltest = LinRange(0.06,0.01, 10)
    p_listchange_finaltest = ones(10) * 0.4 #0.1 prob list change for final test
    # p_listchange_finaltest = ones(10)*0.00 #probe probability change for context

    # criterion_final = LinRange(0.42, 0.60, 10) 
    criterion_final = LinRange(0.22, 0.60, 10)
    # criterion_final = ones(10)*1 
    # 0.42, 0.44, 0.46, 0.48, 0.5, 0.52, 0.54, 0.56, 0.58, 0.6
    # criterion_final =[ 0.5, 0.47, 0.43, 0.43, 0.43, 0.43, 0.33, 0.32, 0.31, 0.3]
    # criterion_final = LinRange(0.45, 0.62, 10) 
    # criterion_final = LinRange(0.32, 0.48, 10) 
    # p_listchange_finaltest = ons(10)*0.06 #probe probability change for context
    final_gap_change = 0;
    ...
    criterion_initial = LinRange(1, 0.45, n_probes);#the bigger the later number, more close hits and CR merges. control merging  
    ...
    n_studytest_change = round.(Int, ones(10) * 7)
    # n_between_listchange =8; #15
    # n_between_listchange =15; #15
    n_between_listchange = 10; #5;15; 
    # n_between_listchange =8; #15
    ...
    u_star = vcat(0.06, ones(n_words) * 0.06)
    ...
    u_star_context = vcat(LinRange(0.1, 0.1, n_grade), ones(n_words - n_grade) .* 0.04) # ratio of this and the next is key for T_nt > T_t, when that for storage and test is seperatly added, also influence
    ...
    function probe_evaluation2(image_pool::Vector{EpisodicImage}, probes::Vector{Probe})::Array{Any}
        ...
        for i in eachindex(probes)
            ...
            if is_restore_final
                restore_intest_final(image_pool, probes[i].image, decision_isold, decision_isold == 1 ? argmax(likelihood_ratios) : 1, probes[i].classification, odds)
            end
        end
    ...
    function probe_evaluation(image_pool::Vector{EpisodicImage}, probes::Vector{Probe}, list_change_features::Vector{Int64}, general_context_features::Vector{Int64},simu_i::Int64)::Array{Any}
        ...
        for i in eachindex(probes)
            ...
            for j in eachindex(unique_list_numbers)
                ...
                nimgPrevStudy = count(ii -> (image_pool_currentlist[ii].list_number == j) && (likelihood_ratios_org[ii] != 344523466743) && (image_pool_currentlist[ii].word.type != :T_foil), eachindex(image_pool_currentlist))
                nimgPrevFoil = count(ii -> (image_pool_currentlist[ii].list_number == j) && (likelihood_ratios_org[ii] != 344523466743) && (image_pool_currentlist[ii].word.type == :T_foil), eachindex(image_pool_currentlist))
                # nimgCurrStudy = count(ii -> (image_pool_currentlist[ii].list_number == unique_list_numbers) && (likelihood_ratios_org[ii] != 344523466743) && (image_pool_currentlist[ii].word.type != :T_foil), eachindex(image_pool_currentlist))
                # println(length(image_pool_currentlist), " ",length(likelihood_ratios_org))
                nimgCurrFoil = count(ii -> (image_pool_currentlist[ii].list_number == length(unique_list_numbers)) && (likelihood_ratios_org[ii] != 344523466743) && (image_pool_currentlist[ii].word.type != :T_foil), eachindex(image_pool_currentlist))
                nimgCurrStudy = count(ii -> (image_pool_currentlist[ii].list_number == length(unique_list_numbers)) && (likelihood_ratios_org[ii] != 344523466743) && (image_pool_currentlist[ii].word.type == :T_foil), eachindex(image_pool_currentlist))

                # println(nimgCurrFoil," ",count(ii -> (image_pool_currentlist[ii].list_number == length(unique_list_numbers))  , eachindex(image_pool_currentlist)))

                results[n_listimagepool*(i-1)+j] = (decision_isold=decision_isold, is_target=probes[i].classification, odds=odds, ilist_image=j, Nratio_imageinlist=nimages_activated / nimages, N_imageinlist=nimages_activated, Nratio_iprobe=nav, testpos=i, studypos=probes[i].image.word.studypos, diff=diff, nimgPrevStudy=nimgPrevStudy, nimgPrevFoil=nimgPrevFoil, nimgCurrStudy=nimgCurrStudy, nimgCurrFoil=nimgCurrFoil, liklihood_N_Nnom = sum(likelihood_ratios), N_Dnom=nl)
            ...
            # Calculate averages based on conditions
            condition1 = [endswith(image_pool_currentlist[iii].word.item , "strg_crr") for iii in eachindex(image_pool_currentlist)]; # Define your first boolean condition array
            condition2 =  [endswith(image_pool_currentlist[iii].word.item , "strg_incrr") for iii in eachindex(image_pool_currentlist)]# is correct recall 
            println("Condition 2 val: ", sum(condition2))

            for iii in eachindex(image_pool_currentlist)
                if (condition1[iii] == true) && (condition2[iii] == true)
                    error("There exists at least one sub-element where condition1 and condition2 are not both true.")
                    println("both true", image_pool_currentlist[iii].word.item)
                end
            end

            imax = argmax([ill==344523466743 ? -Inf : ill for ill in likelihood_ratios_org]);
            println(image_pool[imax])
            if likelihood_ratios_org[imax]==344523466743
                error("This can't be")
                println("There are no images that satisfy condition2")
            end
            println("count if there is any mistaken found item ",likelihood_ratios_org[imax]==344523466743)


            # Satisfy both condition1 and condition2
            avg_c1 = round(mean(likelihood_ratios_org[condition1]), digits=2)
            # println("")
            println("TofF $(probes[i].classification),avg LL for all those correctlly strenghtened: $avg_c1")

            # Satisfy condition1 but not condition2
            avg_c2 = round(mean(likelihood_ratios_org[condition2]),digits=2)
            println("TofF $(probes[i].classification),avg LL for all those INCORRECT strenghtened: $avg_c2")

            if avg_c2==344523466743
                println("")
                println("Currently List: ",probes[i].image.list_number)
                println("current max found in pool ",image_pool[imax].word.item, " ", likelihood_ratios_org[imax], " ,Max liklihood ", likelihood_ratios_org[imax])
                # println("Chosen img ", image_pool_currentlist[condition2].word.item )
                println("check item name ", image_pool_currentlist[condition2][1].word.item)
                println("condition 2 liklihoodval double check ",calculate_likelihood_ratio(probes[i].image.word.word_features ,image_pool_currentlist[condition2][1].word.word_features ,g_word,c) )
                println("condition 2 likelihood val ",likelihood_ratios_org[condition2])
                # error("Wrong") #not wrong because LL is calculated sperately for each probe
                println("There are no images that satisfy condition2")
            end


            # All others (doesn't satisfy condition1)
            avg_others = round(mean(likelihood_ratios_org[.!(condition1 .| condition2)]),digits=2);
            println("TofF $(probes[i].classification),Average likelihood_ratios for all others: $avg_others")


            
            # println("!!!!!!!!!IMGPOOLHERE",[image_pool_currentlist[iii].word.item for iii in eachindex(image_pool_currentlist)])

            if probes[i].classification == :target
                # println(probes[i].classification, probes[i].image.word.studypos, " ", probes[i].image.word.item, " ListNum",probes[i].image.list_number," ", [iimg.word.item for iimg in image_pool_currentlist], " ")
                whichmatch = findfirst(img -> occursin(probes[i].image.word.item, img.word.item), image_pool_currentlist) # Find the first occurrence in the array
                # println(whichmatch)
                println("TRUEMATCH   ",image_pool_currentlist[whichmatch].word.item, " ", probes[i].image.word.item, " ")
                println("ASSUMED MATHCH ", image_pool_currentlist[imax].word.item, " ", probes[i].image.word.item, " ")
                println("Likelihood assumed Max", likelihood_ratios_org[imax])
                

                currlikeli = likelihood_ratios_org[whichmatch]
                println("ToF $(probes[i].classification),Actual T likelihood: $(currlikeli)")
                allothers=filter(iiiratio ->  ((iiiratio != whichmatch) && (iiiratio != 344523466743)), likelihood_ratios_org)
                println("Likelihood avg of all other items ", mean(allothers),", var of others:",var(allothers))    
                println("")
                
x            end

            
            # println("ssssssssss, ",imax)
            println("Comparing likelihood calc, ",calculate_likelihood_ratio(probes[i].image.word.word_features, image_pool[imax].word.word_features, g_word, c)," Likelihood itself from argmax",likelihood_ratios_org[imax])
            println("LengthCompare pool ",length(image_pool)," LL ",length(likelihood_ratios_org))
            println("")
            println("")


            if is_restore_initial
                restore_intest(image_pool, probes[i].image, decision_isold, decision_isold == 1 ? imax : 1, probes[i].classification, list_change_features, general_context_features, odds, likelihood_ratios_org, simu_i) 
            end
    ...
    function restore_intest(image_pool::Vector{EpisodicImage}, iprobe_img::EpisodicImage, decision_isold::Int64, imax::Int64, probetype::Symbol, list_change_features::Vector{Int64}, general_context_features::Vector{Int64}, odds::Float64, likelihood_ratios::Vector{Float64}, simu_i::Int64)
        ...
        # for iimg in eachindex(image_pool)
        #     ll_change_crr = calculate_likelihood_ratio(iprobe_img.word.word_features, image_pool[iimg].word.word_features, g_word, c)
        #     if endswith(image_pool[iimg].word.item, "strg")
        #         push!(ll_change, ll_change_crr)  # Push the calculated likelihood ratio into an array
        #     else
        #         # likelihood_now = calculate_likelihood_ratio(iprobe_img.word.word_features, image_pool[iimg].word.word_features, g_word, c)
        #         push!(ll_unchange, ll_unchange_crr)  # Push the likelihood into the same array
        #     end
        # end

        # println("Number of strenghtened previous traces: $(length(ll_change)),
        # Number of unstrenghtened previous traces: $(length(ll_unchange))")
        # println("avg likelihood traces: $(length(ll_unchange))")
            ...
            for _ in 1:n_units_time_restore
                for i in eachindex(iprobe_img.word.word_features)
                    ...
                    if (j == 0) | ((j != 0) & (decision_isold == 1) & (j != iprobe_img.word.word_features[i]) & (is_store_mismatch))
                        iimage.word.word_features[i] = rand() < u_star[1] ? (rand() < c_storeintest ? iprobe_img.word.word_features[i] : rand(Geometric(g_word)) + 1) : j # 0.04 to u_star_context[2]
                    end
                end
        ...
        #if current is strenghtened, flag 1

        # iflag = Array{Int64}(undef, length(image_pool));
        ...
        if (decision_isold == 1) & (odds > recall_odds_threshold) #single parameter for missing or replacing
            ...
            # end
            # println("pass first stage")
            # if iimage.word.item == iprobe_img.word.item #if the same word, then restore the context features
            # println("check if wrong")

            if iimage.word.item == iprobe_img.word.item
                iimage.word.item = iimage.word.item * "strg_crr";
            else 
                iimage.word.item = iimage.word.item * "strg_incrr";
            end

            # condition2 =  [endswith(image_pool[iii].word.item , "strg_incrr") for iii in eachindex(image_pool)]# is correct recall 
            # avg_c2 = round(mean(likelihood_ratios[condition2]),digits=2)

            if (likelihood_ratios[imax]==344523466743) #if the image is not the same, then restore the word features
                error("This can't be twice")
                println("There are no images that satisfy condition2")
            end

                # println("successfully restore context")


                println("Likelihood Before ",likelihood_ratios[imax])
                println("Likelihood Before, check if same ",calculate_likelihood_ratio(iprobe_img.word.word_features, iimage.word.word_features, g_word, c));;
                ...
                for i in eachindex(iprobe_img.word.word_features)
                    ...
                    if j!=0
                        if (j == 0) | ((j != 0) & (decision_isold == 1) & (j != iprobe_img.word.word_features[i]) & (is_store_mismatch))
                            ...
                            # iflag[imax] = 1;
                            # data_flag[simu_i] = push!(data_flag[simu_i], iimage.word.word_features[i]) #store the changed word features
        ...
        if !is_onlyaddtrace_final
            ...
            if (decision_isold == 1) & (odds > recall_odds_threshold) #single parameter for missing or replacing
                ...
                for i in eachindex(iprobe_img.word.word_features)
                    j = iimage.word.word_features[i]
                    if (j == 0) | ((j != 0) & (decision_isold == 1) & (j != iprobe_img.word.word_features[i]) & (is_store_mismatch))
                        iimage.word.word_features[i] = rand() < 1 ? iprobe_img.word.word_features[i] : j #p_recallFeatureStore
                    end
                end
                # for _ in 1:n_units_time_restore_f
                #     for i in eachindex(iprobe_img.word.word_features)
    ...
    function simulate_rem()
        ...
        df_inital = DataFrame(list_number=Int[], test_position=Int[], simulation_number=Int[], decision_isold=Int[], is_target=Bool[], odds=Float64[], Nratio_iprobe=Float64[], Nratio_iimageinlist=Float64[], N_imageinlist=Float64[], ilist_image=Int[], study_position=Int[], diff_rt=Float64[], nimgPrevStudy=Int[], nimgPrevFoil=Int[], nimgCurrStudy=Int[], nimgCurrFoil=Int[], liklihood_N_Nnom=Float64[],N_Dnom=Int[] )
        ...
        for sim_num in 1:n_simulations
            ...
            for list_num in 1:n_lists
                ...
                for (ires, res) in enumerate(results) #1D array, length is 20 words
                    ...
                    row = [list_num, res.testpos, sim_num, res.decision_isold, tt, res.odds, res.Nratio_iprobe, res.Nratio_imageinlist, res.N_imageinlist, res.ilist_image, res.studypos, res.diff, res.nimgPrevStudy, res.nimgPrevFoil,res.nimgCurrStudy, res.nimgCurrFoil, res.liklihood_N_Nnom, res.N_Dnom] # Add more fields as needed
    ...
    R"""
    DF_now = all_results %>% 
            filter(list_number%in%c(2,3) )%>%
            group_by(list_number, simulation_number, ilist_image, test_position )%>%
            # summarize(meanx = sum(nimgPrevStudy) / N_imageinlist, meany = sum(nimgPrevFoil) / N_imageinlist) %>%
            summarize(meanx = sum(nimgPrevStudy) , meany = sum(nimgPrevFoil) ) %>%
            group_by(list_number,  ilist_image, test_position)%>%
            summarize(meanx=mean(meanx,na.rm=TRUE),meany=mean(meany,na.rm=TRUE))%>%
            pivot_longer(names_to = "StudyFoil", values_to = "nimgPrev", cols = c("meanx","meany"))%>%
            mutate(StudyFoil=case_when(StudyFoil=="meanx"~"nimgPrevStudy",TRUE~"nimgPrevFoil"))%>%
            mutate(list_number=as.factor(list_number) )%>%
            mutate(ilist_image=as.factor(ilist_image))%>%
            mutate(StudyFoil=as.factor(StudyFoil))

    # DF_now2 = DF_now %>%ungroup()%>%
    #         group_by(list_number,ilist_image,test_position)%>%
    #         summarize(avg=mean(nimgPrev))  # Set a larger point size for R graphics device


    p_detail1 = ggplot(data=DF_now, aes(x=test_position,y=nimgPrev)) +
            geom_point(aes(color=ilist_image,shape=StudyFoil,group=interaction(StudyFoil,list_number,ilist_image,ilist_image)),size=8)+
            geom_text(aes(label = ilist_image,color=ilist_image), size = 15, nudge_y = 0.07) +
            geom_line(aes(color=ilist_image,linetype=StudyFoil,group=interaction(StudyFoil,list_number,ilist_image,ilist_image)),size=2,alpha=0.6)+
            # geom_point(data=DF_now2,aes(x=test_position,y=avg ,shape=ilist_image),color="black",size=10)+
            # geom_line(data=DF_now2,aes(x=test_position,y=avg,group=interaction(list_number,ilist_image,ilist_image),linetype=ilist_image),color="black",size=2)+
            facet_grid(list_number~StudyFoil)+
            labs(title="Figure 1. Number of Activated Traces from Previous Lists in Initial Testing")+
            theme(text = element_text(size = 20))  # Increase font size globally

    #next plot how many from current foil,  

    #given target/foil, activated from  CURR  
    DF_now = all_results %>% 
            filter(list_number%in%c(2,3) )%>%
            filter(ilist_image==list_number )%>% 
            # group_by(list_number, test_position,is_target,simulation_number )%>% #sum across ilist_image (from which list)
            # mutate(liklihoodNow = odds*sum(N_imageinlist),nall = sum(N_imageinlist))%>%

            group_by(list_number, simulation_number, test_position,is_target)%>%
            # summarize(meanx = sum(nimgCurrStudy) / N_imageinlist, meany = sum(nimgCurrFoil) / N_imageinlist) %>%
            summarize(meanx = sum(nimgCurrStudy), meany = sum(nimgCurrFoil) ) %>%
            group_by(list_number, test_position,is_target)%>%
            summarize(meanx=mean(meanx,na.rm=TRUE),meany=mean(meany,na.rm=TRUE))%>%
            pivot_longer(names_to = "StudyFoil", values_to = "nimgPrev", cols = c("meanx","meany"))%>%
            mutate(StudyFoil=case_when(StudyFoil=="meanx"~"nimgCurrStudy",TRUE~"nimgCurrFoil"))%>%
            mutate(list_number=as.factor(list_number) )%>%
            # mutate(ilist_image=as.factor(ilist_image))%>%
            mutate(StudyFoil=as.factor(StudyFoil))
    ...
    # DF_now2 = DF_now %>%ungroup()%>%
    #         group_by(list_number,ilist_image,test_position)%>%
    #         summarize(avg=mean(nimgPrev))



    p_detail1_2 = ggplot(data=DF_now, aes(x=test_position,y=nimgPrev)) +
            geom_point(aes(shape=StudyFoil,color=StudyFoil,group=interaction(StudyFoil,list_number,is_target)),size=8)+
            # geom_text(aes(), size = 15, nudge_y = 0.01) +
            geom_line(aes(linetype=is_target,group=interaction(StudyFoil,list_number,is_target)),size=2,alpha=0.6)+
            # geom_point(data=DF_now2,aes(x=test_position,y=avg ,shape=ilist_image),color="black",size=10)+
            # geom_line(data=DF_now2,aes(x=test_position,y=avg,group=interaction(list_number,ilist_image,ilist_image),linetype=ilist_image),color="black",size=2)+
            facet_grid(list_number~.)    +
            labs(title="given target/foil, number of activated from  CURR")+
            theme(text = element_text(size = 20))  # Increase font size globally

    #given target/foil, the avgaraged odds
    DF_now2=all_results%>%
    filter(list_number%in%c(2,3) )%>%
    group_by(list_number, simulation_number, ilist_image, test_position,is_target )%>%
    summarize(meanx = mean(odds)) %>%
    group_by(list_number,  ilist_image, test_position,is_target)%>%
    summarize(avg=mean(meanx,na.rm=TRUE))%>%
    mutate(list_number=as.factor(list_number) )%>%
    mutate(ilist_image=as.factor(ilist_image))%>%
    mutate(coden = case_when(avg>1~"Pass",TRUE~"Fail"))

    DF_now2_supply = DF_now2%>%group_by(list_number, is_target,ilist_image)%>%
            summarize(avgspp = mean(avg))

    p_detail2 = ggplot(data=DF_now2, aes(x=test_position,y=nimgPrev)) +
            geom_point(data=DF_now2,aes(x=test_position,y=avg ,shape=ilist_image,color=interaction(coden,list_number,is_target)),size=10)+
            geom_line(data=DF_now2,aes(x=test_position,y=avg,group=interaction(list_number,ilist_image,ilist_image,is_target),linetype=ilist_image,color=list_number),size=2)+
            # geom_hline(yintercept = 1, linetype = "dashed", color = "black", size = 1.5) +
            geom_hline(data=DF_now2_supply, aes(yintercept=avgspp),color="black", linetype="dashed", size=1.5) +
            facet_grid(list_number~is_target)+
            ylim(c(-1,1e11))+
            labs(title="given target/foil, the avgaraged odds (a reflection of lliklihood as well)")+
            theme(text = element_text(size = 20))  # Increase font size globally
    ...
    DF_now2=all_results%>%
            ...
            mutate(ilist_image=as.factor(ilist_image))%>%
            # mutate(coden = case_when(avg>1~"Pass",TRUE~"Fail"))%>%
            pivot_wider(names_from = "list_number", values_from = "avg")%>%
            mutate(diff = `2`-`3`)%>%
            mutate(coden = case_when(diff>0~"Pass",TRUE~"Fail"))
            
    p_detail2 = ggplot(data=DF_now2, aes(x=test_position,y=diff)) +
            geom_point(data=DF_now2,aes(x=test_position,y=diff ,shape=ilist_image,color=interaction(coden)),size=10)+
            geom_line(data=DF_now2,aes(x=test_position,y=diff,group=interaction(ilist_image,ilist_image,is_target),linetype=ilist_image),size=2)+
            geom_hline(yintercept = 1, linetype = "dashed", color = "black", size = 1.5) +
    ...
    # DF_now2=all_results%>%
    # filter(list_number%in%c(2,3) )%>%
    # # mutate(is_target = case_when(odds>1~"Pass",TRUE~"Fail"))%>%
    # mutate(is_target=decision_isold)%>%
    # group_by(list_number,ilist_image, test_position,is_target )%>%
    # summarize(avg = n()) %>%
    # mutate(list_number=as.factor(list_number) )%>%
    # mutate(ilist_image=as.factor(ilist_image))

    # p_detail3 = ggplot(data=DF_now2, aes(x=test_position,y=avg)) +
    #         geom_point(data=DF_now2,aes(x=test_position,y=avg ,shape=ilist_image,color=interaction(list_number)),size=10)+
    #         geom_line(data=DF_now2,aes(x=test_position,y=avg,group=interaction(list_number,ilist_image,ilist_image,is_target),linetype=ilist_image,color=list_number),size=2)+
    #         # geom_hline(yintercept = 1, linetype = "dashed", color = "black", size = 1.5) +
    #         facet_grid(.~is_target)

    #the following is given decision is old/new, how many are actually target
    # DF_now2=all_results%>%
    # filter(list_number%in%c(2,3) )%>%
    # # mutate(is_target = case_when(odds>1~"Pass",TRUE~"Fail"))%>%
    # mutate(st=is_target,is_target=decision_isold)%>% #st is is_target, is_target is decision_isold
    # group_by(list_number,ilist_image, test_position,is_target )%>%
    # summarize(noverall = n(),nover=sum(st),avg=nover/noverall) %>%
    # mutate(list_number=as.factor(list_number) )%>%
    # mutate(ilist_image=as.factor(ilist_image))
    ...
    #plot given target, sum likelihood and overall N
    DF_now2=all_results%>%
        filter(list_number%in%c(2,3) )%>%
        # mutate(is_target = case_when(odds>1~"Pass",TRUE~"Fail"))%>%
        # mutate(is_target=decision_isold)%>%
        group_by(list_number, test_position,is_target,simulation_number )%>% #sum across ilist_image (from which list)
        mutate(liklihoodNow = mean(liklihood_N_Nnom),nall = mean(N_Dnom))%>%
        # mutate(liklihoodNow = odds*sum(N_imageinlist),nall = sum(N_imageinlist))%>%
        group_by(list_number, test_position,is_target,ilist_image )%>% #sum across ilist_image (from which list)
        summarize(avg = mean(liklihoodNow),nall=mean(nall))%>%
        mutate(list_number=as.factor(list_number) )%>%
        mutate(ilist_image=as.factor(ilist_image))
    ...
    p_detail3 = ggplot(data=DF_now2, aes(x=test_position,y=avg)) +
            ...
            ylim(c(0,1e12))+
            # ylim(c(0,100))+
            labs(title="given target/foil, sum likelihood and overall N")+
            theme(text = element_text(size = 20))+  # Increase font size globally
            geom_hline(data=DF_now2_supply, aes(yintercept=avgspp),color="black", linetype="dashed", size=1.5) +
            facet_grid(list_number~is_target)
    ...
    # calculate in each simlulation, each test position, given target, how many list 3 liklihood are bigger than list 2 liklihood 

    #given target/foil, number of activation in the POOL, 
    # should be the same because first stage are the same for T and F, and their probe context are not different
    DF_now3_2=all_results%>%
        filter(list_number%in%c(2,3) )%>%
        group_by(list_number, test_position,is_target,simulation_number )%>% #sum across ilist_image (from which list)
        # mutate(nall = sum(N_imageinlist))%>%
        mutate(nall = mean(N_Dnom))%>%
        # summarize(nall = 100)%>%
        group_by(list_number, test_position,is_target )%>% #sum across ilist_image (from which list)
        summarize(avg=mean(nall))%>%
        # summarize(avg=100)%>%
        mutate(list_number=as.factor(list_number) )
        # mutate(ilist_image=as.factor(ilist_image))

    p_detail3_2 = ggplot(data=DF_now3_2, aes(x=test_position,y=avg)) +
            geom_point(aes(x=test_position,y=avg ,color=interaction(list_number)),size=10)+
            geom_line(aes(x=test_position,y=avg,group=interaction(list_number,is_target),color=list_number),size=2)+
            # geom_hline(yintercept = 1, linetype = "dashed", color = "black", size = 1.5) +
            facet_grid(.~is_target)+
            labs(title="given target/foil, overall N")+
            theme(text = element_text(size = 20))


    #plot given target/foil, N_imageinlist
    DF_now3_3=all_results%>%
            filter(list_number%in%c(2,3) )%>%
            # mutate(is_target = case_when(odds>1~"Pass",TRUE~"Fail"))%>%
            # mutate(is_target=decision_isold)%>%
            group_by(list_number, test_position,is_target,simulation_number,ilist_image )%>% #sum across ilist_image (from which list)
            mutate(navg = mean(N_imageinlist))%>%
            group_by(list_number, test_position,is_target,ilist_image )%>% #sum across ilist_image (from which list)
            summarize(avg = mean(navg))%>%
            mutate(list_number=as.factor(list_number) )%>%
            mutate(ilist_image=as.factor(ilist_image))
            
    p_detail3_3 = ggplot(data=DF_now3_3, aes(x=test_position,y=avg)) +
            geom_point(aes(x=test_position,y=avg ,shape=ilist_image,color=interaction(ilist_image),group=interaction(list_number,ilist_image,is_target)),size=5)+
            geom_line(aes(x=test_position,y=avg,group=interaction(list_number,ilist_image,is_target),linetype=ilist_image,color=ilist_image),size=2)+
            # geom_hline(yintercept = 1, linetype = "dashed", color = "black", size = 1.5) +
            facet_grid(list_number~is_target)+
            labs(title="avg Num activated in that list from i list")+
            theme(text = element_text(size = 20))  # Increase font size globally
            
    # folowing is given truth of target/foil, how many are judged olld
    DF_now2=all_results%>%
    filter(list_number%in%c(2,3) )%>%
    # mutate(is_target = case_when(odds>1~"Pass",TRUE~"Fail"))%>%
    # mutate(is_target=decision_isold)%>%
    group_by(list_number,ilist_image, test_position,is_target )%>%
    summarize(noverall = n(),nover=sum(decision_isold==1),avg=nover/noverall) %>%
    mutate(list_number=as.factor(list_number) )%>%
    mutate(ilist_image=as.factor(ilist_image))

    p_detail4 = ggplot(data=DF_now2, aes(x=test_position,y=avg)) +
            geom_point(data=DF_now2,aes(x=test_position,y=avg ,shape=ilist_image,color=interaction(list_number)),size=5)+
            geom_line(data=DF_now2,aes(x=test_position,y=avg,group=interaction(list_number,ilist_image,ilist_image,is_target),linetype=ilist_image,color=list_number),size=2)+
            # geom_hline(yintercept = 1, linetype = "dashed", color = "black", size = 1.5) +
            facet_grid(.~is_target)+
            labs(title="given target/foil, simulation avg, how many judged old from N simulation in that list and prioir list i")+
            theme(text = element_text(size = 20))  # Increase font size globally

            # x11(width = 8, height = 6)  # Adjust the width and height as needed

            grid.arrange(p_detail1,p_detail1_2,p_detail2,p_detail3,p_detail3_3,p_detail4,ncol=2,nrow=3)
            # p_detail4
    # ensure_device(1)
    # dev.set(1)  # Target window for Plot 1
    """
    ...
    all_results%>%
            ...
            # mutate(is_target = case_when(odds>1~"Pass",TRUE~"Fail"))%>%
            # mutate(is_target=decision_isold)%>%
            group_by(list_number, test_position,is_target,simulation_number,ilist_image )%>% #sum 
            summarize(navg = mean(N_imageinlist))

    ss=all_results%>%
        filter(list_number%in%c(2,3) )%>%
        # ungroup()%>%
        group_by(list_number, test_position,is_target,simulation_number )%>% #sum across ilist_image (from which list)
        summarize(nall = sum(N_Dnom))%>%
        # mutate(nall = "a")
        # ungroup()
    ...
    p_detail3_2222 = ggplot(data=DF_now3_2222, aes(x=test_position,y=avg)) +
            geom_point(data=DF_now2,aes(x=test_position,y=avg ,color=interaction(list_number,ilist_image)),size=10)+
            geom_line(data=DF_now2,aes(x=test_position,y=avg,group=interaction(list_number,is_target,ilist_image),color=list_number),size=2)+
            # geom_hline(yintercept = 1, linetype = "dashed", color = "black", size = 1.5) +
            facet_grid(.~is_target)+
            labs(title="given target/foil, overall N")+
            theme(text = element_text(size = 20))
