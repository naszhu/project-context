...
criterion_initial = LinRange(1, 0.20, n_probes);#the bigger the later number, more close hits and CR merges. control merging  
...
# -------------------------------
criterion_final = LinRange(0.17, 0.27, 10)
final_gap_change = 0.1;
...
function restore_intest(image_pool::Vector{EpisodicImage}, iprobe_img::EpisodicImage, decision_isold::Int64, imax::Int64, probetype::Symbol, list_change_features::Vector{Int64}, general_context_features::Vector{Int64}, odds::Float64, likelihood_ratios::Vector{Float64}, simu_i::Int64, testpos::Int64)
    ...
    #     rr = counts/10 # 10 probes in each list, here you see how many were strenghtened out of all targets
    #     println("List num $(iprobe_img.list_number), count $counts, ratioStrenghtened $rr")
    # end        
        ...
        for _ in 1:n_units_time_restore
            for i in eachindex(iprobe_img.word.word_features)
                ...
                if (j == 0) | ((j != 0) & (decision_isold == 1) & (j != iprobe_img.word.word_features[i]) & (is_store_mismatch))
                    iimage.word.word_features[i] = rand() < u_star[iprobe_img.list_number] ? (rand() < c_storeintest ? iprobe_img.word.word_features[i] : rand(Geometric(g_word)) + 1) : j # 0.04 to u_star_context[2]
                end
            end
        ...
        for iprobe in eachindex(probe)
            ...
            if probe[iprobe]==:T_target
                ...
                if condition== :true_random
                    ...
                    push!(probes, Probe(EpisodicImage(img.word, crrcontext, img.list_number),:T_target, iprobe))
                else
                    push!(probes, Probe(EpisodicImage(img.word, crrcontext, list_number),:T_target, iprobe))
                end
            elseif probe[iprobe]==:T_nontarget
                ...
                if condition== :true_random
                    ...
                    push!(probes, Probe(EpisodicImage(img.word, crrcontext, img.list_number), :T_nontarget, iprobe))
                else
                    push!(probes, Probe(EpisodicImage(img.word, crrcontext, list_number),:T_nontarget, iprobe))
                end
            elseif probe[iprobe]==:T_foil
                ...
                if condition== :true_random
                    ...
                    push!(probes, Probe(EpisodicImage(img.word, crrcontext, img.list_number),:T_foil, iprobe))
                else
                    push!(probes, Probe(EpisodicImage(img.word, crrcontext, list_number),:T_foil, iprobe))
                end
            elseif probe[iprobe]==:F

                global img = EpisodicImage(Word(randstring(8), rand(Geometric(g_word), w_word) .+ 1, :F, 0), crrcontext, list_number)
...
function simulate_rem()
    ...
    df_final = DataFrame(list_number=Int[], test_position=Int[], simulation_number=Int[], condition=Symbol[], decision_isold=Int[], is_target=String[], odds=Float64[], rt=Float64[], initial_studypos=Int[], initial_testpos=[])
        ...
        if is_finaltest
            for icondition in [:forward, :backward, :true_random]
                ...
                for ii in eachindex(results_final)
                    ...
                    push!(df_final, [res.list_num, ii, sim_num, icondition, res.decision_isold, res.is_target, res.odds, res.rt, res.initial_studypos, res.initial_testpos])
                end
            end
        end
...
if is_finaltest
    ...
    ggplot(data=df_allfinal, aes(test_position_group,val,group=interaction(position_kind,condition,is_target)))+
        ...
        geom_line(aes(color=is_target,group=is_target),size=1)+
        ...
        theme(
            ...
            text=element_text(size=40) # Increase font size globally
        )+
        ylim(c(0.5,1))
