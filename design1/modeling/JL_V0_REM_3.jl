...
w_item = 20; # Number of features
w_context = 20; # Number of features
w = w_item + w_context; # Number of features
g = 0.4; #geometric base rate
g_c = 0.2; #geometric base rate of context
n_units_time = 1; #number of steps
u_star = 0.7; # Probability of storage
c = 0.6; # Probability of copy
n_probes = 20; # Number of probes to test
tau = exp(5); #threshold of liklihood ratio
n_simulations = 1000;
n_lists = 10;
...
# function generate_word_context_vector(w, g)
#     return rand(Geometric(g),w).+1
# end
...
function simulate_rem(g, g_c, c, u_star, w_item, w_context, n_words, n_units_time)
    # Generate word vectors and store episodic images
    words = [generate_word_vector(w_item, g) for _ in 1:n_words]; 
    contexts = [generate_word_vector(w_context, g_c) for _ in 1:n_words];
    episodic_images = [store_episodic_image(n_units_time,u_star,iword , c, g) for iword in words];#(word, c, u_star)
    
    # Simulation results


    return words, contexts, episodic_images
end
# simulate_rem(g, g_c, c, u_star, w_item, w_context, n_words, n_units_time,n_probes)

function simulate_test(words, g, w_item,n_probes)
    ...
    for i in 1:n_probes
        ...
        is_target = rand(Bool);
        probe = choose_probe(words, g, w_item, is_target);
...
function simulate_multiple_lists(n_lists, g, g_c, c, u_star, w_item, w_context, n_words, n_units_time, n_probes, n_simulations)
    ...
    for list_num in 1:n_lists
        for sim_num in 1:n_simulations
            results = simulate_rem(g, g_c, c, u_star, w_item, w_context, n_words, n_units_time);
            # simulate_rem(g, g_c, c, u_star, w_item, w_context, n_words, n_units_time)
...
all_results = simulate_multiple_lists(n_lists, g, g_c, c, u_star, w_item, w_context, n_words, n_units_time, n_probes, n_simulations)
all_results
