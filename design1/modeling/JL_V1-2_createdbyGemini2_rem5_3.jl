...
finished at 3/39/24, change to next version to add learning in final test
...
=========================================================================================================== =#
...
w_context =30; #2*15 number of context features
...
u_star_storeintest = u_star +0.02
c_storeintest = c +0.05 # this is key for T_nt > T_t
u_star_context = u_star 
c_context = c + 0.1
context_tau = 1e5#1e20
context_tau_f = 20;

p_listchange_finalstudy = 0.3; #either of these contribute to differences between list 
p_listchange_finaltest = 0.02;
...
function probe_evaluation2(image_pool, probes)
    ...
    for i in eachindex(probes)
        ...
        results[i] = (decision_isold = decision_isold, is_target = string(probes[i].classification), odds = odds, likelihood_ratios = likelihood_ratios, list_num = probes[i].image.list_number) #! made changes to results, format different than that in inital
    end
    ...
    for _ in 1:n_units_time
        for i in eachindex(iprobe_img.word.word_features)
            ...
            if (j==0) | (j!=0 & decision_isold==1 & j!=iprobe_img.word.word_features[i] & is_store_mismatch)
                stored_val =(rand() < u_star_storeintest ? 1 : 0)*iprobe_img.word.word_features[i];
                if stored_val !=0 #if sucessfully stored do the folowing, else keep the same value
                    # println("now")
                    copied_val = rand() < c_storeintest ? stored_val : rand(Geometric(g_word)) + 1;
                    iimage.word.word_features[i]=copied_val;
                end
            end
        end
        ...
        for ic in eachindex(iprobe_img.context_features)
            ...
            if j==0 | (j!=0 & decision_isold==1 & j!=iprobe_img.context_features[ic].value & is_store_mismatch)# if nothing is stored
                stored_val =(rand() < u_star_context ? 1 : 0)*iprobe_img.context_features[ic].value;
                if stored_val !=0 #if sucessfully stored do the folowing, else keep the same value
                    copied_val = rand() < c_context ? stored_val : rand(Geometric(g_context)) + 1;
                    iimage.context_features[ic].value=copied_val;
                end
            end
