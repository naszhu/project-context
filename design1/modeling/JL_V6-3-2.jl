...
is_restore_final=false#followed by the next
...
p_reinstate_rate = 0.4
    ...
    println("prob of each all features had changed after n $(1-(1-p_reinstate_rate)^5)")
# criterion_initial =  ones(20);
# (1-(1-0.04)^10)
const g_word = 0.4; #geometric base rate
const g_context = 0.3; #geometric base rate of context, or 0.2
...
"""
returns a vector of 30, including study and test only
"""


# First 20 items: 10 random test positions, 10 zeros
...
function generate_study_list(list_num::Int)::Vector{Word}
    ...
    randpos=randperm(20)#random testposition
    first_half = vcat(randpos[1:div(n_words,2)], zeros(Int, div(n_words,2)))|>shuffle!
    last_half = vcat(randpos[div(n_words,2)+1:n_words])
    final_testpos = vcat(first_half, last_half) #first 20 have 0 and 10 testpos, last 10 has 10 testpos
    types=vcat(map(x -> x > 0 ? :T_target : :T_nontarget, first_half),fill(:T_foil,div(n_words,2))...)

    Threads.@threads for i in 1:(n_words+div(n_words,2)) #1:30, 20 study+10foil
        
        i<=20 ? i_studypos=i : i_studypos=0; 
        word_list[i] = Word("Word$(i)L$(list_num)", rand(Geometric(g_word), w_word) .+ 1, types[i],i_studypos, final_testpos[i]) 
    end
...
function store_episodic_image(image_pool::Vector{EpisodicImage}, word::Word, context_features::Vector{Int64}, list_num::Int64)
    ...
    new_image = EpisodicImage(Word(word.item, zeros(Int64, length(word.word_features)), word.type,word.studypos,word.testpos), zeros(length(context_features)), list_num) # Zero word features
...
function generate_probes(studied_words::Vector{Word}, list_change_features::Vector{Int64},test_list_context::Vector{Int64}, general_context_features::Vector{Int64},test_list_context_unchange::Vector{Int64}, position_code_all::Vector{Vector{Int64}},list_num::Int64)::Vector{Probe}
    ...
    # studied_words_study=studied_words[1:n_words];
    # studied_words_foil=studied_words[n_words+1:div(n_words,2)+n_words];

    # probetypes = repeat([:target, :foil], outer = div(n_probes,2)) |>shuffle! ;
    probes = Vector{Probe}(undef, n_probes);
    
    words = filter(word -> word.type == :T_target, studied_words) |> shuffle! |> deepcopy;
    words_foil = filter(word -> word.type == :T_foil, studied_words) |> shuffle! |> deepcopy;
    # println("List $(list_num)")
    for i in 1:n_probes #i is test position
        # println("probe$(i)")
        target_word = filter(w -> w.testpos == i, studied_words) #insert studypos 0
        ...
        current_studypos= target_word.type == :T_target ? target_word.studypos : 0
        current_poscode= target_word.type == :T_target ? position_code_all[current_studypos] : rand(Geometric(g_context),w_positioncode) .+ 1;
        ...
        probes[i] = Probe(EpisodicImage(target_word, current_context_features, list_num), target_word.type, i)
...
function simulate_rem()
    ...
    for sim_num in 1:n_simulations
        ...
        for list_num in 1:n_lists
            ...
            word_list = generate_study_list(list_num);#30 words, first 20 are studied words
            # word_change_context_features = rand(Geometric(g_context),div(w_context, 2)) .+ 1;
            
            for j in 1:n_words #not eachindex(word_list) because it has 30
