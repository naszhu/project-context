#= ===========================================================================================================
   ...
   Notes       : THis is modifed from the saved version, and it wants to try if addding optimization is possible. The LBA part, image print functions is deleted here for clarity and consciseness
                 ...
   =========================================================================================================== =#
...
DF2 = @chain all_results begin
    @by([:list_number, :is_target,:simulation_number], :meanx = mean(:decision_isold))
    @by([:list_number, :is_target], :meanx = mean(:meanx))
    @transform(:meanx = ifelse.(:is_target, :meanx, 1 .- :meanx))
end

innerjoin(DF,DF2, on = [:list_number, :is_target],renamecols = "_left" => "_right")
