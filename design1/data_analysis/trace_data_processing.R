...
# Load the raw data like the CORRECT plotting script does
...
cat("Raw INITIAL testing data before averaging:\n")
raw_check = dfchanged %>%
  filter(task=="pretest_response") %>%
  filter(response!="null") %>%
  group_by(probetype) %>%
...
cat("\n=== CONCLUSION ===\n")
cat("Now I can see if foils are actually higher than targets in the INITIAL testing data!\n")