...
p_reinstate_context = 0 #stop reinstate after how much features

n_studytest_change = round.(Int,ones(10)*0)
...
p_reinstate_rate = 0 #prob of reinstatement
...
u_star = ones(n_words)*0.07
# u_star = ones(n_words)*0.04; # Probability of storage 
u_star_storeintest = u_star #for word

u_star_context = ones(n_words) # ratio of this and the next is key for T_nt > T_t, when that for storage and test is seperatly added, also influence
...
function probe_evaluation(image_pool::Vector{EpisodicImage}, probes::Vector{Probe},list_change_features::Vector{Int64},general_context_features::Vector{Int64} )::Array{Any}
    ...
    unique_list_numbers = unique([image.list_number for image in image_pool]) #this is not rigerous
    ...
    for i in eachindex(probes)
        ...
        # println(nav)
        # println("test",unique_list_numbers)
        for j in eachindex(unique_list_numbers)
            ...
            # Nratio_imageinlist is ratio of images activated in current list, meaning, number of activated current list / number of iamges in that list from the pool   
            ...
            if j==9 && length(unique_list_numbers)==10 
                # println("$(length(likelihood_ratios_org)), $(length(image_pool_currentlist))")
                # println(" Testpos $(probes[i].testpos) s  List $(ilist_probe) $(count(img -> img.list_number == j, image_pool_currentlist)) found $(count(ii ->  ((likelihood_ratios_org[ii] != 344523466743) && (image_pool_currentlist[ii].list_number == 1) ), eachindex(image_pool_currentlist))) $(length(image_pool_currentlist))")
            end
        end
        ...
        if is_restore_initial
            restore_intest(image_pool,probes[i].image, decision_isold, decision_isold==1 ? argmax(likelihood_ratios) : 1, probes[i].classification, list_change_features,general_context_features, odds);
        end
...
"""
function restore_intest(image_pool::Vector{EpisodicImage}, iprobe_img::EpisodicImage, decision_isold::Int64,imax::Int64, probetype::Symbol,list_change_features::Vector{Int64},general_context_features::Vector{Int64}, odds::Float64)

    if is_onlyaddtrace error("not coded here") end
    ...
    if (decision_isold == 0) 
        ...
        for _ in 1:n_units_time_restore
            for i in eachindex(iprobe_img.word.word_features)
                ...
                if (j==0) 
                    iimage.word.word_features[i] = rand() < u_star_context[2] ? (rand() < c_storeintest ? iprobe_img.word.word_features[i] : rand(Geometric(g_word)) + 1) : j; # 0.04 to u_star_context[2]
                end
            end
    ...
    if (decision_isold == 0) 
        # println("ss",iimage.list_number==10)
...
function restore_intest_final(image_pool::Vector{EpisodicImage}, iprobe_img::EpisodicImage, decision_isold::Int64,imax::Int64, probetype::Symbol, odds::Float64)
    ...
    if (decision_isold == 0) 
        ...
        for _ in 1:n_units_time_restore
            for i in eachindex(iprobe_img.word.word_features)
                j=iimage.word.word_features[i]
                if (j==0) | ((j!=0) &( decision_isold==1) & (j!=iprobe_img.word.word_features[i]) & (is_store_mismatch))
                    iimage.word.word_features[i] = rand() < 0.04 ? (rand() < c_storeintest ? iprobe_img.word.word_features[i] : rand(Geometric(g_word)) + 1) : j; # 0.04 to u_star_context[2]
                end
            end
    ...
    if (decision_isold == 1) & (odds>recall_odds_threshold) #single parameter for missing or replacing
        ...
        if !is_store_mismatch error("current prog is not written when doesn't store mismatch") 
        end
...
"""Input the flattened studied pool, first 30 are t/n/f in list 1, and etc; give last list's list_change_cf to change from list to list for probes
    studied_pool: a flatten array of images, each image is a list of 30 images (10 Tt, 10 Tn, 10 Tf) of 10 lists
    """
function generate_finalt_probes(studied_pool::Array{EpisodicImage}, condition::Symbol,general_context_features::Vector{Int64},list_change_context_features::Vector{Int64})::Vector{Probe}
    ...
    # push each image into the corresponding list in the dictionary
    for img in studied_pool
        push!(get!(list_images, img.list_number, Vector{EpisodicImage}()), img)
    end
    # println(length(studied_pool), " images in the pool")
    # println("initial after pop ",length(list_images[1]))
    
    # lists is the list number of the images in the pool; 
    lists = keys(list_images) |> collect |>sort;
    probes = Vector{Probe}()
    if condition == :backward
        lists = reverse(lists); #[10,9,8...]
    elseif condition == :true_random #[just 1]
        
        # true random doesn't change list context during final test
        all_images = fast_concat(collect(values(list_images)))  # Combine all lists using fast_concat
    ...
    # println("current condition $(condition), probes $([iprb.image.list_number for iprb in probes])")
