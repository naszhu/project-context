...
++++++++++
6-4： first stage of "filtering" context content together 
=========================================================================================================== =#
...
# p_listchange_finaltest = LinRange(0.2,0 , 10)
p_listchange_finaltest = ones(10)*0.02
...
w_word = 25; # number of word features, 30 optimal for inital test, 25 for fianal, lower w would lower overall accuracy 
is_firststage = true;
...
criterion_initial =  vcat(LinRange(1, 0.5, 20),ones(n_probes-min(20,n_probes)).-0.4) #change by test position
...
# n_listchange = 5;
# n_RI_change = 15;#20
# n_RI_change = round.(Int, range(5, stop=35, length=10))
# n_RI_change = round.(Int, range(2, stop=35, length=10)) 
# n_RI_change = round.(Int,ones(10)*25)
n_RI_change = round.(Int,ones(10)*8)
# n_listchange =8; #15
# n_listchange =15; #15
n_listchange =5; #15
# n_listchange =8; #15
const p_listchange = 0.03; # studied prior list probability change 
#n_ri and n_listchange use p_listchange at the same time
p_reinstate_rate = 0.2#0.4 #prob of reinstatement
    # n_RI_change = round.(Int,ones(10)*25)
    println("prob of each feature change between list $(1-(1-p_listchange)^n_listchange)")
    println("prob of each feature drift between study and test $(1-(1-p_listchange)^n_RI_change[1])")
    aa=(1-(1-p_listchange)^n_listchange);
...
const g_context = g_word; #0.3 geometric base rate of context, or 0.2
# const u_star = 0.04; # Probability of storage 
n_grade=2 #only first to be special 
# u_star = vcat(LinRange(0.08, 0.04, n_grade),ones(n_words-n_grade).*0.04); # Probability of storage 
u_star = ones(n_words)*0.04; # Probability of storage 
u_star_storeintest = u_star #for word
# u_star_context = vcat(LinRange(0.08, 0.04, n_grade),ones(n_words-n_grade).*0.04) # ratio of this and the next is key for T_nt > T_t, when that for storage and test is seperatly added, also influence
u_star_context = vcat(LinRange(0.2, 0.04, n_grade),ones(n_words-n_grade).*0.04) # ratio of this and the next is key for T_nt > T_t, when that for storage and test is seperatly added, also influence
...
function generate_study_list(list_num::Int)::Vector{Word}
    ...
    types = vcat(fill.([:T_target, :T_nontarget], [Int(n_probes/2),Int(n_probes/2)])...) |>shuffle! ;
...
function generate_probes(studied_words::Vector{Word}, list_change_features::Vector{Int64},test_list_context::Vector{Int64}, general_context_features::Vector{Int64},test_list_context_unchange::Vector{Int64}, position_code_all::Vector{Vector{Int64}},list_num::Int64)::Vector{Probe}
    ...
    for i in eachindex(probetypes)
        ...
        current_context_features = vcat(deepcopy(test_list_context_unchange), deepcopy(test_list_context),current_poscode) ; #here needs a deepcopy, otherwise the front remembered context change with later ones  
...
Big change in this function from las version
    change context_likelihoods, 
"""
function calculate_two_step_likelihoods(probe::EpisodicImage, image_pool::Vector{EpisodicImage}, p::Float64, iprobe::Int64)::Tuple{Vector{Float64}, Vector{Float64}} 
    all_likelihoods = Vector{Float64}(undef,length(image_pool));
    ...
    for ii in eachindex(image_pool) 
        ...
        if is_firststage #this is barely used in current version 
            
            if is_test_allcontext  #here is secon  stage would be wrong, including position code, unchage, change
                all_likelihood = calculate_likelihood_ratio(vcat(probe.word.word_features[1:round(Int, w_word * p)],probe_context...), vcat(image.word.word_features[1:round(Int, w_word * p)],image_context...), g_word, c) #gword=gcontext here

            else  #not testing all context but change only, no unchange or position code
                error("not changed here 2")
            ...
            all_likelihoods[ii] = all_likelihood

            # second stage
            if all_likelihood > all_tau # if pass context criterion 
    ...
    return all_likelihoods, word_likelihoods
end
...
function calculate_two_step_likelihoods2(probe::EpisodicImage, image_pool::Vector{EpisodicImage}, p::Float64, iprobe::Int64)::Tuple{Vector{Float64}, Vector{Float64}} 
    ...
    word_likelihoods = Vector{Float64}(undef,length(image_pool));
    probe_context = probe.context_features;
    probe_context_f=vcat(probe_context[1:nU_f],probe_context[nU_f+1:nU_f+nC_f]...)
    
    for ii in eachindex(image_pool) 
        image = image_pool[ii];
        image_context = image.context_features;
        if is_test_allcontext2  #here is secon  stage would be wrong
            image_context_f=vcat(image_context[1:nU_f],image_context[nU_f+1:nU_f+nC_f]...)
            all_likelihood = calculate_likelihood_ratio(vcat(probe.word.word_features,probe_context_f...),vcat(image.word.word_features,image_context_f...),g_context,c)  # .#  Context calculation
        elseif is_test_changecontext2  
            all_likelihood = calculate_likelihood_ratio(vcat(probe.word.word_features,probe_context[nU+1:end]...),vcat(image.word.word_features,image_context[nU+1:end]...),g_context,c)
        else #only test general context (first part)
            all_likelihood = calculate_likelihood_ratio(vcat(probe.word.word_features,probe_context[1:nU]...),vcat(image.word.word_features,image_context[1:nU]...),g_context,c)
            # calculate_likelihood_ratio(probe_context[1:nU],image_context[1:nU],g_context,c )  # .#  Context calculation
        end
        ...
        if is_firststage
            ...
            # second stage
            if all_likelihood > all_tau_final # if pass context criterion 
    ...
    for _ in 1:n_units_time_restore
        ...
        for i in eachindex(iprobe_img.word.word_features)
            ...
            if decision_isold==1
                ...
                if (j==0) | ((j!=0) &( decision_isold==1) & (j!=iprobe_img.word.word_features[i]) & (is_store_mismatch))
                    iimage.word.word_features[i] = rand() < 0.04 ? (rand() < c_storeintest ? iprobe_img.word.word_features[i] : rand(Geometric(g_word)) + 1) : j;
                end
            end
        end

        # iimage.word.word_features[tf] = ifelse.(rand(sum(tf)) .< 0.04, ifelse.(rand(sum(tf)) .< c_storeintest, iprobe_img.word.word_features[tf], rand(Geometric(g_context),sum(tf)) .+ 1), iimage.word.word_features[tf])
        # iimage.word.word_features[tf] = ifelse.(rand(sum(tf)) .< 1, ifelse.(rand(sum(tf)) .< 1, iprobe_img.word.word_features[tf], rand(Geometric(g_context),sum(tf)) .+ 1), iimage.word.word_features[tf])
        # iimage.word.word_features[tf]=iprobe_img.word.word_features[tf]
        # iimage.word.word_features[tf] = ifelse.(rand(sum(tf)) .< 1, ifelse.(rand(sum(tf)) .< 1, iprobe_img.word.word_features[tf], rand(Geometric(g_context),sum(tf)) .+ 1), iimage.word.word_features[tf])
        ...
        if is_restore_context
            ...
            for ic in eachindex(iprobe_img.context_features)
                ...
                # if decision_isold == 1 #crr_all_contextf, store study context for all old
                    # iimage.context_features[ic] = rand() < u_star_context ? (rand() < c_context ? crr_all_contextf[ic] : rand(Geometric(g_context)) + 1) : j;
                    # iimage.context_features[ic] =  iprobe_img.context_features[ic]
                    # println("here")
                    
                if false
                else (j==0) | ((j!=0) & (decision_isold==1) & (j!=iprobe_img.context_features[ic]) & is_store_mismatch & (!is_onlyaddtrace))# if nothing is stored
                    # println(j,!is_onlyaddtrace)
    ...
    for sim_num in 1:n_simulations
        ...
        for list_num in 1:n_lists
            ...
            for j in eachindex(word_list) 
                ...
                current_context_features = vcat(deepcopy(general_context_features), deepcopy(list_change_context_features),position_code_features_study);
            ...
            for _ in 1:n_RI_change[list_num] #context drift
        ...
        for ccf in eachindex(general_context_features)
            if rand() <  0.35 #cf.change_probability # this equals p_change
