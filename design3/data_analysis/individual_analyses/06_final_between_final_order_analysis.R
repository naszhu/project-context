...
results <- broom.mixed::tidy(m_final_between_final_e3, effects = "fixed", conf.int = TRUE)
...
print(as.data.frame(final_position_emmeans))
...
saveRDS(
  list(
    model = m_final_between_final_e3,
    summary = results,
    linear_trends = as.data.frame(final_position_lin_trend),
    quadratic_trends = as.data.frame(final_position_quad_trend),
    emmeans = as.data.frame(final_position_emmeans),
    pairwise = as.data.frame(final_position_pairs)
  ),
  "final_between_final_model.rds"
)

# Export summary as CSV
results %>%
  mutate(model = "final_between_final") %>%
  write_csv("final_between_final_summary.csv")

cat("\nSaved: final_between_final_model.rds\n")
cat("Saved: final_between_final_summary.csv\n")
