...
df2 <-
  ...
  mutate(
    probe_history = case_when(
      (n_tests == 1) & (stimulusConditions %in% c("Cn","Fn")) ~   "foil",
      (n_tests == 1) & (stimulusConditions %in% c("B"))       ~   "target", 
      (n_tests == 2) & (stimulusConditions %in% c("Fnn"))     ~   "foil(n)->foil(n+1)",
      (n_tests == 2) & (stimulusConditions %in% c("Dn"))      ~   "target(n)->foil(n+1)", 
  ...
  ################# ⬅ EDIT 1: new factor for facet rows
  mutate(
    bin_size = case_when(
      init_outcome %in% c("C","E")   ~ "single",
      init_outcome %in% c("C1&C2", "C1&E2","E1&C2","E1&E2") ~ "double-joint"
    ) 
  )
...
# 2) Aggregate ----------------------------------------------------------
agg <- df2 %>%
  group_by(n_tests, group_init, probe_history, init_outcome, bin_size) %>%
  summarize(
  ...
  #                               TRUE ~ probe_history))%>%
  filter(complete.cases(group_init) & complete.cases(probe_history), complete.cases(init_outcome))%>%
...
ggplot(agg, aes(x = group_init, y = prob_final,
               group = interaction(n_tests, probe_history, init_outcome, bin_size))) +
  geom_line(aes(color =init_outcome, linetype=probe_history )) +
  geom_point(aes(color= init_outcome,  shape = probe_history),size=3) +
  facet_grid( . ~ n_tests+bin_size,
