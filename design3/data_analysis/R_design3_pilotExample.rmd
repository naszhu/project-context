...
agg <- df2 %>%
  ...
  # mutate(median_ref = median(mean_perf, na.rm=TRUE)) %>%
   # judge assign group
    mutate(rank_id = dense_rank(mean_perf)
           )%>%
  #find median number of the rank so could seprate 
  ungroup()%>%
  mutate(median_rank = median(rank_id, na.rm = TRUE))%>%
  ...
  ### now calculate the mean of the sub mean
