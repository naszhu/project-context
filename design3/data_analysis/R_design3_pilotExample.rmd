...
# 2) Aggregate ----------------------------------------------------------
agg <- df2 %>%
  group_by(n_tests, group_init, probe_history, init_outcome) %>% #new fac add or not addhere
  summarize(
...
ggplot(agg, aes(x = group_init, y = prob_final,
  ...
  facet_grid( . ~ n_tests+probe_history,
             labeller = labeller(n_tests = c(`1`="Tested once", `2`="Tested twice"))) +
  labs(
    x = "Initial-list bin",
    y = "P(final correct)",
    # color = "Outcome",
    title = "Recognition performance by list-bin and outcome"
  ) 
  # scale_color_manual(
  #   values = c(
...
# d1taf = df_pl_with_init%>%
  ...
  #add calculation of sd (or se?) as well below 
         # summarise(cr = median(correct),se_cr = sd(correct,na.rm = TRUE) / sqrt(n()))%>%
