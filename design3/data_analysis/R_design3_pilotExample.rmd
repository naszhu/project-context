...
# 0.009 (~1% data deon'st have initial score, prob they all are those that didn't have an rt (null)
  ...
  # filter(!( !complete.cases(correct_appear1,correct_appear2) & ! (stimulusConditions %in% c("FF","A")) ))%>%
     # select(task,subject_id,id_picName,stimulusConditions,correct_appear1,correct_appear2,listNum_appear1_initial.x,listNum_appear1_initial.y, listNum_appear2_initial)


# df_rt_pl %>% filter(task=="initialTest_response")%>%
#   # filter(!(stimulusConditions%in%c("A") ))%>%
#    filter(correct_appear1=="null",correct_appear2=="null")
# 4% data is null , prob they all are those that didn't have an rt (null)
# df_pl_with_init%>%
#   filter(!(stimulusConditions%in%c("A","FF") ))%>% #filter out that of conditon study only
#     select(subject_id,id_picName,stimulusConditions,correct_appear1,correct_appear2,listNum_appear1_initial)%>%
#   filter(!complete.cases(correct_appear1,correct_appear2))

# df_rt_pl%>%filter(subject_id=="66436e0c091888a5fc85584e" & id_picName=="1 (2170).jpg")%>% #filter out that of conditon study only
#     select(task,subject_id,id_picName,stimulusConditions,correct_appear1,correct_appear2,listNum_appear1_initial)
# 
# 
# df_pl%>%filter(subject_id=="66436e0c091888a5fc85584e" & listNum_appear0_initial==3)%>% #filter out that of conditon study only
#   filter(task=="initialTest_response")%>%
#     select(task,subject_id,id_picName,rt, correct,stimulusConditions,correct_appear1,correct_appear2,listNum_appear1_initial)
# df_pl%>%filter(subject_id=="67cb5baef51d73572cb490ae")
```
