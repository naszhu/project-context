...
function simulate_rem()
    ...
    df_final = DataFrame(list_number = Int[], test_position = Int[], simulation_number = Int[], condition = Symbol[], decision_isold = Int[], is_target = String[],odds = Float64[],criterion = Float64[])
    ...
    for sim_num in 1:n_simulations
        ...
        criterion_finals = 0.4:0.2:1.6|>collect;
        #final test here
        
        for icr in criterion_finals
            println(sim_num,icr)
            global criterion_final = icr;
            for icondition in [:forward, :backward, :true_random]
                image_pool_backup=deepcopy(image_pool);
                finalprobes = generate_finalt_probes(studied_pool, icondition,general_context_features,list_change_context_features);
                results_final = probe_evaluation2(image_pool_backup, finalprobes);
                for ii in eachindex(results_final)
                    res = results_final[ii]
                    push!(df_final, [res.list_num, ii, sim_num, icondition, res.decision_isold, res.is_target, res.odds,icr])
                end 
            end
        end
...
DFf = @chain allresf begin
    @by([:list_number, :is_target, :test_position, :condition, :simulation_number, :icr], :meanx = mean(:decision_isold))
    @by([:list_number, :is_target, :test_position, :condition, :icr], :meanx = mean(:meanx))
...
# summarize(meanx=mean(meanx))
# ggplot(data=DF2, aes(x=list_number,y=meanx))+
# geom_point(aes(color=is_target))+
# geom_line(aes(color=is_target))
...
DF22 = allresf %>% 
mutate(crr = case_when(is_target!="F"~ decision_isold, TRUE ~ 1-decision_isold))%>%
mutate(test_position=as.numeric(test_position))%>%
# mutate(criterion=as.factor(criterion))%>%
# mutate(test_position_in_chunks = cut_number(test_position,n=10))%>%
group_by(condition,simulation_number,criterion)%>%
mutate(meanx = mean(crr))%>%
group_by(condition,criterion)%>%
summarize(meanx = mean(meanx))
head(DF22)

ggplot(data=DF22, aes(x=criterion,y=meanx))+
...
# mutate(criterion=as.factor(criterion))%>%
# mutate(test_position_in_chunks = cut_number(test_position,n=10))%>%
# group_by(test_position_in_chunks,condition,simulation_number,criterion)%>%
# mutate(meanx = mean(crr))%>%
# group_by(test_position_in_chunks,condition,criterion)%>%
# summarize(meanx = mean(meanx))
# head(DF22)

# ggplot(data=DF22, aes(x=test_position_in_chunks,y=meanx,group=criterion))+
# geom_point(aes(color=criterion))+
# geom_line(aes(color=criterion))+
# facet_grid(condition~.)+
# # ylim(0.76,0.9)+
# scale_x_discrete(guide = guide_axis(angle = 45))
...
# mutate(criterion=as.factor(criterion))%>%
# mutate(test_position_in_chunks = cut_number(test_position,n=10))%>%
# group_by(test_position_in_chunks,condition,simulation_number,criterion,is_target)%>%
# mutate(meanx = mean(decision_isold))%>%
# group_by(test_position_in_chunks,condition,criterion,is_target)%>%
...
# geom_point(aes(color=criterion))+
# geom_line(aes(color=criterion))+
# facet_grid(condition~is_target)+
# # ylim(0.76,0.9)+
# scale_x_discrete(guide = guide_axis(angle = 45))
