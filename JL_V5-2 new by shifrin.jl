...
n_simulations= 50
is_restore_context = false # currently don't want to restore context
context_tau = 100
...
function calculate_two_step_likelihoods(probe::EpisodicImage, image_pool::Vector{EpisodicImage}, p::Float64, iprobe::Int64)::Tuple{Vector{Float64}, Vector{Float64}} 
    ...
    for ii in eachindex(image_pool) 
        ...
        if is_test_allcontext  #here is second stage would be wrong
            ...
            context_likelihood = calculate_likelihood_ratio(probe_context[(w_context ÷ 2) + 1 : end],image_context[(w_context ÷ 2) + 1 : end],g_context,c )  # .#  Context calculation
        end
