#= ===========================================================================================================
   ...
   Notes       : add new way of plotting, changed structure 

   later: please check if the true and false in restore function is good 
                 ...
   =========================================================================================================== =#
...
const p_listchange_finaltest = 0.2;                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            
...
mutable struct EpisodicImage
    ...
    list_number::Int64
    type_storage:: Int64 #1:study storage. 2: test streghten 3: test added 0: probe image 4: newly generated study image
end
...
function store_episodic_image(image_pool::Vector{EpisodicImage}, word::Word, context_features::Vector{Int64}, list_num::Int64)
    ...
    new_image = EpisodicImage(Word(word.item, zeros(Int64, length(word.word_features)), word.type), zeros(length(context_features)), list_num, 1) # Zero word features, 1 means study image
...
function generate_probes(studied_words::Vector{Word}, list_change_features::Vector{Int64}, general_context_features::Vector{Int64}, list_num::Int64)::Vector{Probe}
    ...
    Threads.@threads for i in eachindex(probetypes)
        ...
        probes[i] = Probe(EpisodicImage(target_word, current_context_features, list_num, 0), probetypes[i], i)
...
function probe_evaluation(image_pool::Vector{EpisodicImage}, probes::Vector{Probe})::Array{Any}
    ...
    # results = Array{Any}(undef, n_probes*length(image_pool)); #this was previously wrong
    results = []; #this was previously wrong
    ...
    for i in eachindex(probes)
        ...
        # results[(n_listimagepool-1)*length(probes)+i] = (decision_isold = decision_isold, is_target = probes[i].classification, odds = odds,Nratio_imageinlist = nimages_activated, Nratio_iprobe = nav, testpos = i);
        ...
        #     #in order to attach number of images activated for each list, each line is now expanded to n lines 
        #     nimages = count(image -> image.list_number == j, image_pool); #number total image in pool 
        #     nimages_activated=count(ii -> (image_pool[ii].list_number == j) && (likelihood_ratios_org[ii] != 344523466743), eachindex(image_pool));

        #     results[n_listimagepool*(i-1)+j] = (decision_isold = decision_isold, is_target = probes[i].classification, odds = odds, ilist_image=j,Nratio_imageinlist = nimages_activated, Nratio_iprobe = nav, testpos = i, type_storage = );
        # end


        for i_img in eachindex(image_pool) #now we have to go through everyimage and append the same result to every image because we have to take result of whether the iamge was from strengthened or not. This expansion will not influence the aggregated data result
            
            #in order to attach number of images activated for each list, each line is now expanded to n lines 
            i_listnumber = image_pool[i_img].list_number;
            # println(length(likelihood_ratios_org),"and",length(image_pool))
            
            nimages = count(image -> image.list_number == i_listnumber, image_pool); #number total image in pool 
            nimages_activated=count(ii -> (image_pool[ii].list_number == i_listnumber) && (likelihood_ratios_org[ii] != 344523466743), eachindex(image_pool));

            is_activated = !(likelihood_ratios_org[i_img] == 344523466743);
            type_storage = image_pool[i_img].type_storage

            # println(length(image_pool))
            # println("nimagepool$(length(image_pool)),nprobes$(length(probes)),sum$(length(results))")
            # println(length(image_pool),"and",i,"and",i_img,"and", length(image_pool)*(i-1)+i_img, length(results))
            push!(results,(decision_isold = decision_isold, is_target = probes[i].classification, odds = odds, ilist_image=image_pool[i_img].list_number,nimages_activated=nimages_activated ,Nratio_imageinlist = nimages_activated/nimages, Nratio_iprobe = nav, testpos = i, type_storage = type_storage, is_activated=is_activated, iprobe=i, iimage=i_img));
        end

        imax = argmax(likelihood_ratios);
        restore_intest(image_pool,probes[i].image, decision_isold, decision_isold==1 ? imax : 1, probes[i].classification);
...
function restore_intest(image_pool::Vector{EpisodicImage}, iprobe_img::EpisodicImage, decision_isold::Int64,imax::Int64, probetype::Symbol)
    ...
    # 3 is test added, 2 is test strengthen
    if is_onlyaddtrace #only start with empty if only add trace
        iimage = EpisodicImage(Word(iprobe_img.word.item, fill(0, length(iprobe_img.word.word_features)), iprobe_img.word.type), zeros(length(iprobe_img.context_features)), iprobe_img.list_number, 3);
    else
        # println("nothere")
        iimage = decision_isold ==  1 ? image_pool[imax] : EpisodicImage(Word(iprobe_img.word.item, fill(0, length(iprobe_img.word.word_features)), iprobe_img.word.type), zeros(length(iprobe_img.context_features)), iprobe_img.list_number, 3); 

        if decision_isold==1
            iimage.type_storage=2;
        end
    ...
    for list_number in lists
        ...
        for iprobe in eachindex(probe)  
            ...
            if probe[iprobe]==:T_target
                push!(probes, Probe(EpisodicImage(pop!(images_Tt).word, crrcontext, list_number,0),:T_target, iprobe))
            elseif probe[iprobe]==:T_nontarget
                push!(probes, Probe(EpisodicImage(pop!(images_Tnt).word, crrcontext, list_number,0),:T_nontarget, iprobe))
            elseif probe[iprobe]==:T_foil
                push!(probes, Probe(EpisodicImage(pop!(images_Tf).word, crrcontext, list_number,0),:T_foil, iprobe))
            elseif probe[iprobe]==:F
                push!(probes, Probe(EpisodicImage(Word(randstring(8), rand(Geometric(g_word), w_word) .+ 1, :F), crrcontext, list_number,0),:F, iprobe))  # Generate a new foil
            else
...
function simulate_rem()
    ...
    # push!(results,(decision_isold = decision_isold, is_target = probes[i].classification, odds = odds, ilist_image=image_pool[i_img].list_number
# nimages_activated=nimages_activated ,
# Nratio_imageinlist = nimages_activated/nimages, Nratio_iprobe = nav, testpos = i, 
# type_storage = type_storage, is_activated=is_activated));

    df_inital = DataFrame(list_number = Int[], test_position = Int[], simulation_number = Int[], decision_isold = Int[], is_target = Bool[],odds = Float64[], Nratio_iprobe = Float64[], Nratio_iimageinlist = Float64[], ilist_image = Int[], nimages_activated=Int64[], type_storage=Int64[], is_activated=Bool[], iprobe=Int64[], iimg=Int64[])
    ...
    for sim_num in 1:n_simulations
        ...
        for list_num in 1:n_lists
            ...
            for j in eachindex(word_list) 
                ...
                episodic_image = EpisodicImage(word_list[j], current_context_features, list_num, 4);
            ...
            for (ires, res) in enumerate(results) #1D array, length is 20 words
                ...
                row = [list_num, res.testpos, sim_num, res.decision_isold, tt, res.odds, res.Nratio_iprobe, res.Nratio_imageinlist, res.ilist_image, res.nimages_activated, res.type_storage, res.is_activated, res.iprobe, res.iimage] # Add more fields as needed


                # DataFrame(list_number = Int[], test_position = Int[], simulation_number = Int[], decision_isold = Int[], is_target = Bool[],odds = Float64[], Nratio_iprobe = Float64[], Nratio_iimageinlist = Float64[], ilist_image = Int[], nimages_activated=Int64[], type_storage=Int64[], is_activated=Bool[])
                push!(df_inital, row)
            end
...
DF4 = all_results %>% 
filter(is_activated)%>%
group_by(list_number, simulation_number, ilist_image,type_storage,iprobe)%>%
summarize(numx=n())%>% #for each probe, activated image in each list 
group_by(list_number, simulation_number, ilist_image,type_storage)%>%
summarize(meanx=mean(numx))%>% #averaged across probe
group_by(list_number, ilist_image,type_storage)%>%
summarize(meanx=mean(meanx))%>%
mutate(list_number=as.factor(list_number),type_storage=as.factor(type_storage))%>%
mutate(type_storage=case_when(type_storage==1~"studied image",
                                type_storage==2~"test strengthen",
                                type_storage==3~"test added"))%>%
filter(list_number%in%c(1,5,10))
# mutate(ilist_image=as.factor(ilist_image))
# mutate(test_position=as.factor(test_position))

p5=ggplot(data=DF4, aes(x=ilist_image,y=meanx, group=interaction(list_number,type_storage)))+
geom_point(aes(color=type_storage))+
geom_line(aes(color=type_storage))+
# geom_text(aes(label = round(meanx,digits=3)),nudge_y = 0.01)+
labs(title="Number of activated trace for list 1,5,10",y="number of activated trace")+
# scale_x_continuous(name="list",breaks = rev(1:10),labels=as.character(rev(1:10)))
scale_x_reverse(name="traces from which list, left end - recent list, right, right end - prior list",breaks = 1:10,labels=as.character(1:10)) +
facet_grid(list_number~.)
...
mutate(type_storage=case_when(type_storage==1~"studied image",
                                type_storage==2~"test strengthen",
                                type_storage==3~"test added"))%>%
filter(list_number==10)
# mutate(ilist_image=as.factor(ilist_image))
# mutate(test_position=as.factor(test_position))

p6=ggplot(data=DF4, aes(x=ilist_image,y=meanx, group=interaction(list_number,type_storage)))+
geom_point(aes(color=type_storage))+
geom_line(aes(color=type_storage))+
# geom_text(aes(label = round(meanx,digits=3)),nudge_y = 0.01)+
labs(title="Ratio of activated trace of 10 lists",y="Ratio of activated trace")+
# scale_x_continuous(name="list",breaks = rev(1:10),labels=as.character(rev(1:10)))
scale_x_reverse(name="traces from which list, left end - recent list, right, right end - prior list",breaks = 1:10,labels=as.character(1:10)) +
facet_grid(list_number~.)
...
grid.arrange(p1, p4,p5,p6 ,ncol = 2,nrow=2)
"""