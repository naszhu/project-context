...
n_grade=4
# u_star = ones; # Probability of storage 
u_star = ones(n_words)*0.04; # Probability of storage 
u_star_storeintest = u_star #for word
# u_star_context = u_star # ratio of this and the next is key for T_nt > T_t, when that for storage and test is seperatly added, also influence
u_star_context = vcat(LinRange(0.06, 0.04, n_grade),ones(n_words-n_grade).*0.04);  # ratio of this and the next is key for T_nt > T_t, when that for storage and test is seperatly added, also influence
...
"""
function generate_probes(studied_words::Vector{Word}, list_change_features::Vector{Int64},test_list_context::Vector{Int64}, general_context_features::Vector{Int64}, position_code_all::Vector{Vector{Int64}},list_num::Int64)::Vector{Probe}
    ...
    for i in eachindex(probetypes)
        ...
        current_context_features = vcat(general_context_features, deepcopy(test_list_context),current_poscode) ; #here needs a deepcopy, otherwise the front remembered context change with later ones  
...
function probe_evaluation(image_pool::Vector{EpisodicImage}, probes::Vector{Probe},list_change_features::Vector{Int64},general_context_features::Vector{Int64} )::Array{Any}
    ...
    for i in eachindex(probes)
        ...
        decision_isold = odds > criterion_initial[i_testpos] ? 1 : 0;
        diff = 1/abs(odds-criterion_initial[i_testpos])
        ...
        if is_restore_context
            ...
            for ic in eachindex(iprobe_img.context_features)
                ...
                if false
                    ...
                    iimage.context_features[ic] = rand() < 0.04 ? (rand() < c_context ? iprobe_img.context_features[ic] : rand(Geometric(g_context)) + 1) : j;
                end
    ...
    for sim_num in 1:n_simulations
        ...
        for list_num in 1:n_lists
            ...
            for j in eachindex(word_list) 
                ...
                current_context_features = vcat(general_context_features, deepcopy(list_change_context_features),position_code_features_study);
            ...
            probes = generate_probes(word_list, list_change_context_features,test_list_context,general_context_features, position_code_all,list_num); #probe number is current list number, get probes of current list 
            # println("list $(list_num), ")
