...
3. try adding a filter for inital  serial-position result (no output interfereance) 
...
is_firststage = true;
is_finaltest = false;
is_onlyaddtrace = false; #*add but not strengtening trace; false=add+strengten
is_onlytest_currentlist=false;#true=not accumulating traces between lists
const g_word = 0.5; #geometric base rate
...
const p_listchange = 0.05; # studied prior list probability change 
...
const p_listchange_finaltest =0.01 #probe probability change for context


#only probing with change context in inital test， but this should be changed if a second stage is added 
is_test_allcontext = false #not testing all context in intial test
...
Return: probe
"""
function generate_probes(studied_words::Vector{Word}, list_change_features::Vector{Int64},word_change_features::Vector{Int64}, general_context_features::Vector{Int64}, list_num::Int64)::Vector{Probe}
    ...
    for i in eachindex(probetypes)
        ...
        for cf in eachindex(word_change_features)
            if rand() <  p_wordchange #cf.change_probability # this equals p_change
                word_change_features[cf] = rand(Geometric(g_context)) + 1 
            end
        end

        current_context_features = vcat(general_context_features, list_change_features,deepcopy(word_change_features)) ; #here needs a deepcopy, otherwise the front remembered context change with later ones  
...
function calculate_two_step_likelihoods(probe::EpisodicImage, image_pool::Vector{EpisodicImage}, p::Float64, iprobe::Int64)::Tuple{Vector{Float64}, Vector{Float64}} 
    ...
    for ii in eachindex(image_pool) 
        ...
        if is_test_allcontext  #here is second stage would be wrong
            context_likelihood = calculate_likelihood_ratio(probe_context,image_context,g_context,c )  # .#  Context calculation
        else  #not testing all context; the last 2/3 half are list_change and word_change context
            context_likelihood = calculate_likelihood_ratio(probe_context[(w_context ÷ 3) + 1 : end],image_context[(w_context ÷ 3) + 1 : end],g_context,c )  # .#  Context calculation
        end
...
function probe_evaluation2(image_pool::Vector{EpisodicImage}, probes::Vector{Probe})::Array{Any}
    ...
    for i in eachindex(probes)
        ...
        _, likelihood_ratios = calculate_two_step_likelihoods(probes[i].image, image_pool, 1.0,i)
...
function probe_evaluation(image_pool::Vector{EpisodicImage}, probes::Vector{Probe})::Array{Any}
    ...
    for i in eachindex(probes)
        ...
        if is_onlytest_currentlist
            ...
            # println(length(image_pool_currentlist))
        else
            image_pool_currentlist=image_pool;
        end
            # println("this is list $(currentlist),there are $(length(image_pool_currentlist)) images in the current pool")
        ...
        # criterion in inital test
        if ilist_probe==1
            decision_isold = odds > 0.8 ? 1 : 0;
        else
...
function simulate_rem()
    ...
    for sim_num in 1:n_simulations
        ...
        general_context_features = rand(Geometric(g_context),div(w_context, 3)) .+ 1#[ContextFeature(rand(Geometric(g_context)) + 1, :general, p_change) for _ in 1:div(w_context, 2)] 
        list_change_context_features = rand(Geometric(g_context),div(w_context, 3)) .+ 1#[ContextFeature(rand(Geometric(g_context)) + 1, :list_change, p_change) for _ in 1:div(w_context, 2)]
        
        for list_num in 1:n_lists
            
            word_list = generate_study_list(list_num);
            word_change_context_features = rand(Geometric(g_context),div(w_context, 3)) .+ 1;
            
            for j in eachindex(word_list) 
                current_context_features = vcat(general_context_features, deepcopy(list_change_context_features),deepcopy(word_change_context_features));
            ...
            probes = generate_probes(word_list, deepcopy(list_change_context_features), deepcopy(word_change_context_features),general_context_features, list_num); #probe number is current list number, get probes of current list 
...
df2=all_results%>%mutate(is_target=case_when(is_target~1,TRUE~0),correct=decision_isold==is_target)%>%
    group_by(test_position,is_target,simulation_number,ilist_image)%>%
    summarize(meanx=mean(correct))%>%
    group_by(test_position,is_target,ilist_image)%>%
    summarize(meanx=mean(meanx))%>%
    mutate(is_target=as.factor(is_target))

p_in_20in10=ggplot(data=df2,aes(x=test_position,y=meanx,group=interaction(ilist_image,is_target)))+
    geom_line(aes(color=is_target))+
    geom_point()+
    facet_grid(ilist_image~.)

p_in_20in10

df3=all_results%>%mutate(is_target=case_when(is_target~1,TRUE~0),correct=decision_isold==is_target)%>%
    mutate(listbreak=case_when(ilist_image<=5~1,TRUE~2))%>%
...
R"""
grid.arrange(p4,p1,p_in_20in10_break2,p_in_20,p_in_20in10,ncol = 2,nrow=3)
