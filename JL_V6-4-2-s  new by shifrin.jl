...
6-4-2-s: this version is what edited on pc outside home. 
(1) found that the logic of is_onlyaddtrace condition in pushing a new array is possibly wrong in restore_intest;
(2) add 1 argument in restore_intest: odds; (this is for adding/use a threshold test when judge old/recall) 
(3) add variable recall_odds_threshold; p_recallFeatureStore
=========================================================================================================== =#
# using Pkg
...
is_onlyaddtrace_final=true #Warning: this is possibly wrong in later implimentation  
...
is_onlyaddtrace = false; ##Warning: this is possibly wrong in later implimentation 
...
n_studytest_change = round.(Int,ones(10)*13) #each feature drift between study and test
# n_between_listchange =8; #15
# n_between_listchange =15; #15
n_between_listchange =5; #5;15; prob of feature change during test
# n_between_listchange =8; #15
...
n_units_time_restore = n_units_time + 10 #this only applies for new 
...
function restore_intest(image_pool::Vector{EpisodicImage}, iprobe_img::EpisodicImage, decision_isold::Int64,imax::Int64, probetype::Symbol,list_change_features::Vector{Int64},general_context_features::Vector{Int64},odds::Float64)
    ...
    if is_onlyaddtrace & decision_isold == 0#only start with empty if only add trace, no strengtening below
    ...
    if (decision_isold == 0) 
        ...
        for _ in 1:n_units_time_restore
            ...
            if is_restore_context
                error("context restored in initial is not well written this part")
            ...
            else #currently, context is not restored, but only added
                # error("context not restored in initial")
                # do nothing
            end
        end
    end
    ...
    if (decision_isold == 1) & (odds>recall_odds_threshold) #single parameter for missing or replacing
        ...
        for i in eachindex(iprobe_img.word.word_features)
            j=iimage.word.word_features[i]
            if (j==0) | ((j!=0) &( decision_isold==1) & (j!=iprobe_img.word.word_features[i]) & (is_store_mismatch))
                iimage.word.word_features[i] = rand() < p_recallFeatureStore ? iprobe_img.word.word_features[i] : j;
            end
        end
