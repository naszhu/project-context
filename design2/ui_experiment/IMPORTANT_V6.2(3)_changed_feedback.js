 ...
 * Date: 2023-12-26
 *
 * Description: codeversion 6_2/3
 * intially I forgot to change codeversion, there were 2 subs fr without that, and then 6_2 codeversion 
 * with 2br, 
 * and then I changed an error in code (I forgot to add 1):  
 * thus the final test position was incorrect, new version after change is 6.3
 * const totdigits = range(1,num_finaltest+1);//digits 0-120?
 * this influenced 56caf7b564c068000c4d798a 650afc00d748aa554ebde221
 * Add looping function for final test instructions 
 * change the feedback form totally
 * changed the algorithm to calculate indent, add condition fr, br
 * ...
 ****************************************************************************************************************/
...
///////////////////////////////
    ...
    if (is_debug) {
      ...
      var feedbackmes_wordinitial_duration = 500; 
      var feedbackmes_wordfinal_duration = 1500; 
...
//****************************************************
    ...
    <p style='text-align: justify;color: black;background: white;font: caption-;''>(2)<strong>After a brief blank period, you will see a series of single digits.</strong> Add those as they come, and when you see the words “TYPE THE SUM”, use the number keys to type your answer. </p>
    <p style='text-align: justify;color: black;background: white;font: caption-;''>(3) <strong>Next you will be tested to assess your memory for the words from the list you JUST studied.</strong> You will be shown <strong>the first two letters</strong> of one of the words in that list, and try to remember that word. </p>
    ...
    <p style='text-align: justify;color: black;background: white;font: caption-;''>(5)You will see several recall tests like this for each list, each with different first two letters. </p>
    ...
    if (condi==='f'){mes = `<strong>There will be 10 sets of cues presented. You will see a list number presented before each set of cues, indicating the list in which the words were studied from the 10 lists you studied. </strong> <br>
        (1) The first cues will be for words seen in the first list studied, list number 1. You will see first the message: “LIST NUMBER 1”. <br>
        (2) The next set of cues will be for list number 2, and you will first see the messages “LIST NUMBER 2”, and so on until the cues for list number 10 will be given. <br>
        (3) Note that sometimes the cue letters will also have a different word in the surronding list, and you should type both words in the spaces provided if you can recall both.`}
    else if (condi==="b"){mes=`<strong>There will be 10 sets of cues presented. You will see a list number presented before each set of cues, indicating the list in which the words were studied from the 10 lists you studied. </strong> <br>
    ...
    else if (condi==="fr" || condi==="br") {mes = `<strong>There will be 10 sets of cues presented. You will see a list number presented before each set of cues, indicating the list in which the words were studied from the 10 lists you studied. </strong>`}
    else if (condi==="r") (mes='')
    ...
    finalint_text = `<p style='text-align: justify;color: black;background: white;font: caption-;paddingRight =30px; paddingLeft =30px;'>
        ...
        The next part contains only tests of words. It will test for <strong>ALL</strong> the words you have seen in this study. <br><br>

        You will see two letter cues, just as before, but now there will be <strong>two spaces</strong> to type words beginning with those two letters: <br>

        <FONT color= '#CC5500'>For  <strong>some</strong> cues you will have seen <strong>ONLY ONE WORD</strong> beginning with those letters; and for <strong>some</strong> cues you will have seen <strong>TWO WORDS</strong> beginning with those letters. <br></FONT>
        ...
        (4) After both spaces are filled, you will be told the correct words, and 'none' means no second word was studied.<br><br>
    ...
    * ...
    ...
    stimulus: function(){
        ...
        console.log(listnum)
        if (condi !== "r"){
            return (`Words from <strong>LIST ${listnum}</strong> and its adjecent list`)
        }
    },
    choices: "NO_KEYS",
    ...
    questions: function(){
        ...
        prompt: "".concat(`<p style="line-height: 1.5;">`,`Type <strong>the whole word</strong> you studied in <strong>ALL</strong> previous lists, starting with <strong>"`,ftlnow,`</strong>`,`________"</p><p style="line-height: 1.5;"> 
        Press 'enter' key to go to next blank or next cues</p>`,`<p style="line-height: 1.5;">The first word if you remember:</p>`), //changed!!
        placeholder:``.concat(`type the word starting with '`,JT('ftl'),`' or 'none'`),
        required: true
        }, 
        {
    ...
    on_finish: function(data){
        ...
        data.is_repeat_word1_reverse = data.word1_response === data.wordchosen_finaltest[1];//didn't filter out unique condition
        data.is_repeat_word2_reverse = data.word2_response === data.wordchosen_finaltest[0] && !data.is_repeatanswer;
        ...
        if (data.is_repeat_word2_reverse && !data.is_repeatanswer){//second word correct and not repeat answer
            ...
            data.is_correct_first = data.wordchosen_finaltest.includes(data.word1_response) && true; //convert to boollen dummy code
            data.is_correct_second = data.wordchosen_finaltest.includes(data.word2_response) && !data.is_repeatanswer;//changed
        }

        // console.log(data.is_correct_first,data.is_correct_second)
        ...
        data.is_correct_repeat_precise = data.wordcondi==="repeat" ? data.is_correct_word1_precise&&data.is_correct_word2_precise: undefined;
        data.is_correct_avg_repeat_precise = data.wordcondi==="repeat" ? average([data.is_correct_word1_precise,data.is_correct_word2_precise]) : undefined;
        data.is_correct_unique_precise = data.wordcondi==="unique" ? data.is_correct_word1_precise || data.is_correct_word2_precise : undefined; //unique correct when one of those is correct
        // console.log(data.is_correct_unique_precise,data.is_correct_avg_repeat_precise)
    ...
    stimulus: function(){
        ...
        iscorrect1=""
        iscorrect2=""
        warn_fill=`. <span style="background-color: yellow;">Please prioritize in filling the first blank!</span>`;

        concatfont = struse=>''.concat(`<FONT color='#B22222'>`,struse,`</FONT>`);
        concathighlight = struse=>''.concat(`<span style="background-color: yellow;">`,struse,`</span>`);

        if (lasttrial_obj.wordcondi==="repeat"){
            
            if (lasttrial_obj.is_correct_word1_precise && !lasttrial_obj.is_repeat_word1_reverse) {//first correct, and match first word
                iscorrect1+=lasttrial_obj.wordchosen_finaltest[0];
                iscorrect2+=lasttrial_obj.wordchosen_finaltest[1];
            }else if (lasttrial_obj.is_correct_word1_precise && lasttrial_obj.is_repeat_word1_reverse){//first correct, and match second word
                iscorrect1+=lasttrial_obj.wordchosen_finaltest[1];
                iscorrect2+=lasttrial_obj.wordchosen_finaltest[0];
            }else if (!lasttrial_obj.is_correct_word1_precise && lasttrial_obj.is_repeat_word2_reverse){//first incorrect, second correct match first word{}
                iscorrect1+=lasttrial_obj.wordchosen_finaltest[1];
                iscorrect2+=lasttrial_obj.wordchosen_finaltest[0];
            }else{//first incorrect and second incorrect and so on
                iscorrect1+=lasttrial_obj.wordchosen_finaltest[0];
                iscorrect2+=lasttrial_obj.wordchosen_finaltest[1];
            }

        } else if (lasttrial_obj.wordcondi==="unique") {

            if (lasttrial_obj.is_correct_word1_precise) {//word1correct
                iscorrect1+=lasttrial_obj.wordchosen_finaltest[0];
                iscorrect2+="none";
            } else if (lasttrial_obj.is_repeat_word2_reverse){//word2correct
                iscorrect2+=lasttrial_obj.wordchosen_finaltest[0];
                iscorrect1+="none" ;
            } else if (!lasttrial_obj.is_correct_word1_precise && !lasttrial_obj.is_repeat_word2_reverse){
                iscorrect1+=lasttrial_obj.wordchosen_finaltest[0];
                iscorrect2+="none";
            }            
        }

        crword1=lasttrial_obj.word_response_original[0];
        crword2=lasttrial_obj.word_response_original[1];
        if (!lasttrial_obj.is_correct_first) {crword1=concathighlight(crword1);iscorrect1=concathighlight(iscorrect1);}
        if (!lasttrial_obj.is_correct_second) {crword2=concathighlight(crword2);iscorrect2= concathighlight(iscorrect2);}

        if (lasttrial_obj.word1_response==="none" && lasttrial_obj.word2_response!=="none") iscorrect1 +=warn_fill
        ...
        responsefeed1 = ``.concat(answer1,`<p style="margin-bottom: 42px;">`,"Answer:<u> <strong>",crword1, ` </u></strong>&nbsp&nbsp&nbsp&nbsp Correct Answer: <strong>`,iscorrect1,`</strong></p>`)
        responsefeed2 = ``.concat(answer2,`<p style="margin-bottom: 23px;">`,"Answer:<u> <strong>", crword2,` </u></strong>&nbsp&nbsp&nbsp&nbsp Correct Answer: <strong>`,iscorrect2,`</strong></p>`)

        if (!lasttrial_obj.is_correct_first) responsefeed1=concatfont(responsefeed1)
        if (!lasttrial_obj.is_correct_second) responsefeed2=concatfont(responsefeed2)
    ...
    on_start: function(trial){
        ...
        jsPsych.data.addProperties({
            all_accumulated_accuracy: average([...firstcr,...secondcr,...initialcr])
        });
    },
    type: jsPsychHtmlKeyboardResponse,
    stimulus: function(){
        ...
        secondcr = jsPsych.data.get().select('is_correct_second').values;
        initialcr = jsPsych.data.get().select('correct').values;
        nownow =  average([...firstcr,...secondcr,...initialcr]);
        console.log(nownow)
