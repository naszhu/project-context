var is_debug = false;
    ...
    var jsPsych = initJsPsych({
        ...
        on_finish: function() {
            ...
            if (jsPsych.data.get().last(4).trials[0].testpos==420) {
                window.location = "https://app.prolific.co/submissions/complete?cc=C1BNHEB2";
                }
            }
    });
    jsPsych.data.addProperties({
 ...
 //same as above, but created to only used in for loop below
    ...
    var preloass= {
        type: jsPsychPreload,
        images: picnames_random.slice(0,num_pretest_tot+num_finalt_foil).map(i_pic=>picdir+i_pic).concat(picdir+"blank.png",picdir+"correct.jpg",picdir+"Incorrect.jpg"),
    ...
    var instructions_finaltest = {
      type: jsPsychHtmlKeyboardResponse,
      stimulus: finalint_text.concat("<p style='text-align: justify;color: black;background: white;font: caption-; paddingRight =20px; paddingLeft =20px''>start the test list with the ‘enter’ key</p>"),
      post_trial_gap: posgap_duration,
      trial_duration: instruction_duration,
      choices: choiceenter
    };
    ...
    var v_finaltest = {
       ...
       prompt: function(){
            ...
            //   return '<p color: black;background: white;><strong>"F" for new&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;"J" for old</strong></p>'.concat(
            //       "Your accumulated accuracy for this trial is: <strong><FONT color=#C41E3A>").concat(
            //     Math.round(lastresp.accumulated_accuracy*10000)/100).concat("%</FONT></strong>")
            ...
            // if (lastresp.task == "finalt_response"){
            //   feedbacknow = "Your accumulated accuracy for this trial is: <FONT color=#C41E3A>".concat(
            //       Math.round(lastresp.accumulated_accuracy*10000)/100,"%</FONT></strong>",`<p><FONT color="white"> . <FONT></p>`)
            // }
            // else { //first trial
            //   feedbacknow = "Your accumulated accuracy for this trial is: ".concat(`<p><FONT color="white"> . </p><FONT>`)
            // }
