  ...
  class SurveyTextPlugin {
      ...
      trial(display_element, trial) {
          ...
          display_element.innerHTML = html;
            ...
            // Add callback for keydown events on input changed!
            var key_presses = [];
            var key_rt = [];
            var record_startT = null;
            var record_endT;
            var time_passed;

            document.getElementById("input-0").addEventListener("keydown", (e) => {

                time_passed = performance.now() - startTime;
                key_presses.push(e.key);
                key_rt.push(time_passed);
                var isLetter = /^[a-zA-Z]$/.test(e.key);
                if (isLetter) {
                    if (!record_startT) record_startT = time_passed;
                    record_endT = time_passed;
                } 
            })
          ...
          display_element.querySelector("#jspsych-survey-text-form").addEventListener("submit", (e) => {
              ...
              var trialdata = {
                  rt: response_time,
                  rt_keypresses: key_rt.map(i=>Math.round(i)),
                  response: question_data,
                  keypresses: key_presses,
                  rt_key_start: Math.round(record_startT),
                  rt_key_end: Math.round(record_endT)
              };
