 ...
 * Date: 2024-01-02
 *
 * Description: V10: this version is a main change that used only 1 answer in final test, and it detects time when first start to type word
 * added property of finaltest group in data
 * ...
 ****************************************************************************************************************/
...
///////////////////////////////
    ...
    const codeversion_begins = 10;//!!
    // const timelimit_finalinst=1000*60;
    // const timelimit_initial_inst = 1000*60;
    const timelimit_finalinst=1000*30;
    const timelimit_initial_inst = 1000*30;
    // const timelimit_finalinst=1000;//!!
    ...
    const num_finaltest = ntf*num_listtotest;
...
//****************************************************
    ...
    // num_pairlists_usedinFinal=4
    // ntf: number of test in final test each = 15
    ...
    const backward_final_ind = range(0,num_pairlists_usedinFinal).reverse().map(i=>[].concat(JRR(range(i*ntf+ntf+ntf*i+1,i*ntf+ntf+ntf+ntf*i+1),1),JRR(range(i*ntf+ntf*i+1,i*ntf+ntf+ntf*i+1),1)));
    ...
    const totdigits = range(1,num_finaltest+1);//digits 0-120?
    const randomizedset = range(0,num_listtotest).map(ilist=>JRR(totdigits.slice(ilist*ntf,ilist*ntf+ntf),1));
    const [frandom_final_ind, brandom_final_ind] = combinedReorderArray(randomizedset);
    console.log(randomizedset,frandom_final_ind)
    
    // const frandom_final_ind_before = range(0,num_listtotest).map(ilist=>JRR(totdigits.slice(ilist*ntf,ilist*ntf+ntf),1))
    // const frandom_final_ind = reorderArray(frandom_final_ind_before)
    // console.log("now",frandom_final_ind)
    console.log("forward,backward, and randomind, fr and br",forward_final_ind,backward_final_ind,random_final_ind,frandom_final_ind,brandom_final_ind)
    // const psudorandom_forward_final = 
    ...
    else if (condi==="fr") finalt_ind = frandom_final_ind;
    else if (condi==="br") finalt_ind = brandom_final_ind;
    // finalt_ind = finalt_ind.flat();
    finalt_ind = range(0,num_listtotest).map(ilist=>finalt_ind.flat().slice(ilist*15,ilist*15+15))//15 tests per list
    // console.log(finalt_ind)
    console.log("finaltest ident:",finalt_ind)


    //****************************************************
    //                    Add final test position for tot_word_arobj_nf_rd_slice
    //****************************************************
    arraynow=[...tot_word_arobj_nf_rd_slice];
    alluniqueftl=arraynow.map(ilist=>ilist.filter(obj=>obj.wordcondi=="unique").map(i=>i.ftl)).flat().filter((item, index, array) => {
        return array.indexOf(item) === index;})
    allrepeatftl=arraynow.map(ilist=>ilist.filter(obj=>obj.wordcondi=="repeat").map(i=>i.ftl)).flat().filter((item, index, array) => {
        return array.indexOf(item) === index;})
    allrepeatftl_inpairs = range(0,num_listtotest/2).map(ilistpair=>JRR(allrepeatftl.slice(ilistpair*10,ilistpair*10+10),1))
    // console.log(allrepeatftl,allrepeatftl_inpairs)

    finalt_alltestftl_uq=range(0,num_listtotest).map(ilist=>{

        ilistpair=Math.floor(ilist/2);
        pairswhich = ilist % 2; 
        return(
            ftl_unique_set=JRR(alluniqueftl.slice(ilist*10,ilist*10+10),1)//10 unique tested
           
        )
    })

    finalt_alltestftl_rp=range(0,num_listtotest).map(ilist=>{
        ilistpair=Math.floor(ilist/2);
        pairswhich = ilist % 2; 
        return(
            
            ftl_repeat_set=allrepeatftl_inpairs[ilistpair].slice(pairswhich*5,pairswhich*5+5)//5 repeat tested
        )
    })
    // console.log(finalt_alltestftl_uq)



    
    arraynow.forEach(function(ilist,list_ind){

        // console.log(list_ind,finalt_alltestftl_uq[list_ind])
        var jcount = 0;
        // for (i_unique_ftl in finalt_alltestftl_uq[list_ind])
        finalt_alltestftl_uq[list_ind].forEach(i_unique_ftl =>{

            iobjnow = ilist.find(obj=>obj.ftl===i_unique_ftl);
            iobjnow.istestedfinal = "unique_tested";
            // console.log(finalt_ind,ilist,jcount)
            iobjnow.testpos_final = finalt_ind[list_ind][jcount];
            iobjnow.is_chosenfinaltest = 1;
            // console.log(iobjnow.testpos_final,i_unique_ftl)
            jcount++
        });
        // for (i_repeat_ftl in finalt_alltestftl_rp[list_ind]){
        finalt_alltestftl_rp[list_ind].forEach(i_repeat_ftl=>{

            iobjnow = ilist.find(obj=>obj.ftl===i_repeat_ftl);
            if (list_ind%2===0) {
                iobjpair=arraynow[list_ind+1].find(obj=>obj.ftl===i_repeat_ftl);
            }else{
                iobjpair=arraynow[list_ind-1].find(obj=>obj.ftl===i_repeat_ftl);
            }
            // console.log(iobjpair)
            iobjnow.istestedfinal = "repeat_tested";
            iobjnow.is_chosenfinaltest = 1;
            iobjpair.is_chosenfinaltest = 0;
            iobjpair.istestedfinal = "repeat_notested";
            iobjnow.testpos_final = finalt_ind[list_ind][jcount];
            iobjpair.testpos_final = finalt_ind[list_ind][jcount];
            jcount++;
        });
    ...
    // console.log("here",arraynow.flat().filter(i=>i.istestedfinal!=="repeat_notested").map(i=>[i.testpos_final,i.ftl,i.listnumber_1to10,i.wordcondi,i.istestedfinal]))
    ...
    // console.log("listnumber,wordcondition,testposition,presentposition",tot_word_finaltest_arobj.filter(i=>i.wordcondi=="repeat").map(i=>[i.listnumber_1to10,i.ftl,i.wordcondi,i.testpos_final,i.prespos]))
    console.log("listnumber,wordcondition,testposition,presentposition",tot_word_finaltest_arobj.map(i=>[i.listnumber_1to10,i.ftl,i.wordcondi,i.testpos_final,i.prespos]))

    
    //****************************************************
    //                    add group number based on final test order
    //****************************************************
    range(0,num_listtotest).map(i=> tot_word_finaltest_arobj.slice(i*ntf,i*ntf+ntf).map(j_obj=>j_obj.listgroup_finaltest=i+1))
    tot_word_arobj_nf_rd_slice.flat().forEach((iobj,i_ind)=>{
        if (iobj.listgroup_finaltest&&iobj.wordcondi==="repeat"){
            ifound = tot_word_arobj_nf_rd_slice.flat().find(i=>i.ftl===iobj.ftl&&i.listnumber_1to10!==iobj.listnumber_1to10);
            // console.log(ifound)
            ifound.listgroup_finaltest=iobj.listgroup_finaltest
        }
    })

    // testpos_final
    //****************************************************
    //                    following are algorithm check
    //****************************************************
    //check if there is any repeats: none
    // console.log([...new Set(tot_word_finaltest_arobj.map(i=>i.ftl))])

    //check if the condition are randomized by claculating average of presentation position
    let keynow=5//5 for repeat condition, 10 for unique, 15 for all
    console.log(range(0,8).map(ii=>tot_word_finaltest_arobj.filter(i=>i.wordcondi=="repeat").map(i=>i.prespos).slice(ii*keynow,ii*keynow+keynow)).map(i=>average(i)).map(i=>(i-10).toFixed(3)))//get mean of presentation order to see if its' randomaized
    ...
    practice_text = `<p style='text-align: justify;color: black;background: white;font: caption-;''><strong>You will engage with 8 lists of words,</strong> each comprising 20 words for study and 8 words for testing.  Between the study and test phases, you will be required to count single digits:
        
        </p>
        <p style='text-align: justify;color: black;background: white;font: caption-;''>&nbsp&nbsp&nbsp&nbsp(1) <strong>Study: A list of words will appear on the screen one at a time.</strong> Study the words as they appear.</p>
        <p style='text-align: justify;color: black;background: white;font: caption-;''>&nbsp&nbsp&nbsp&nbsp(2) <strong>Counting: After a brief blank period, you will see a series of single digits.</strong> Add those as they come, and when you see the words “TYPE THE SUM”, use the number keys to type your answer. </p>
        <p style='text-align: justify;color: black;background: white;font: caption-;''>&nbsp&nbsp&nbsp&nbsp(3) <strong>Test: Next you will be tested to assess your memory for the words from the list you JUST studied.</strong> You will be shown <strong>the first two letters</strong> of one of the words in that list, and try to remember that word. </p>
            <p style='text-align: justify;color: black;background: white;font: caption-;''>&nbsp&nbsp&nbsp&nbsp(4) <strong>If you can recall it</strong>, type the entire word, including the first two letters, in the space provided. You will be told if you were correct. <strong>If you cannot remember</strong>, then type the word 'none'. You will see several recall tests like this, each with different first two letters.  </p>
            
        
        <p style='text-align: justify;color: black;background: white;font: caption-;''>&nbsp&nbsp&nbsp&nbsp(5) Following the same format as above, you will see with <strong>8 word lists</strong>, each involving stages of study, counting, and test.  </p> <br>
        ...
        <p style='text-align: justify;color: black;background: white;font: caption-;''> Please start the first list now by pressing the RETURN/ENTER key. <strong>Be prepared to remember the words as they appear.</strong> </p>
    ...
    var instructions_between = {
        ...
        stimulus: `
        <p style='text-align: justify;color: black;background: white;font: caption-;''>
            Press the 'enter' key to see the next list of words. Be prepared to remember the words as they appear. </p>
        `,
        post_trial_gap: posgap_duration,
    ...
    if (condi==='f'){mes = `<strong>There will be 8 sets of cues presented. You will see A LIST NUMBER presented before each set of cues,</strong> indicating the list in which the words were studied from the 8 lists you studied. The sets are tested in a forward order:   <br><br>
        <strong>(1)</strong> The first set of cues will be for words seen in the first list studied, list number 1. You will see first the message: “LIST NUMBER 1”. <br>
        <strong>(2)</strong> The next set of cues will be for list number 2, and you will first see the messages “LIST NUMBER 2”, and so on until the cues for list number 8 will be given. <br>
        <strong>(3)</strong> <strong>Note that sometimes the cue letters will also have a different word in the surrounding lists.</strong> You should try to type the word from the list with the number provided, but if you are not sure, type any word you remember beginning with those letters`}
    else if (condi==="b"){mes=`<strong>There will be 8 sets of cues presented. You will see A LIST NUMBER presented before each set of cues,</strong> indicating the list in which the words were studied from the 8 lists you studied. The sets are tested in a backward order:  <br><br>
    <strong>(1)</strong> The first set of cues will be for words seen in the last list studied, list number 8, and you will first see the message “LIST NUMBER 8”.<br> 
    <strong>(2)</strong> The next set of cues will be for the second to last list, list number 7, and you will first see the message “LIST NUMBER 7”, and so on until the cues for list number 1 will be given. <br>
    <strong>(3)</strong> <strong>Note that sometimes the cue letters will also have a different word in the surrounding lists. </strong>You should try to type the word from the list with the number provided, but if you are not sure, type any word you remember beginning with those letters`}
    else if (condi==="fr" || condi==="br") {mes = `<strong>There will be 8 sets of cues presented. You will see A LIST NUMBER presented before each set of cues, indicating the list in which the words were studied from the 8 lists you studied. </strong><br>
        
        <strong>Note that sometimes the cue letters will also have a different word in the surrounding lists. </strong>You should try to type the word from the list with the number provided, but if you are not sure, type any word you remember beginning with those letters
        `}
    else if (condi==="r") (mes='')
    ...
    // Adding the looping trial to the timeline
    ...
    for (let i_trial=0; i_trial<num_listtotest; i_trial++){
        ...
        var v_study = {
            ...
            data: {
                ...
                listgroup_finaltest: JT('listgroup_finaltest'),
        ...
        var v_test = {
            ...
            on_start:function(trial){
                ...
                is_showcorrect_inlog?console.log(i_trial,JT('wordchosen_initial'),JT('ftl'),JT('wordchosen_finaltest'),JT('word_left_i'),JT('word_right_i')):null;
                if (is_debug){
            ...
            on_finish: function(data){
                ...
                data.is_notremembered = data.word_response_extract == "none";
                data.correct = data.word_response_extract == data.wordchosen_initial;
                
                if (data.pairs_which_1or2===2){//on the 'right'
                    data.is_correct_otherlist = data.word_response_extract == data.wordchosen_finaltest[0];
                    // console.log(data.is_correct_otherlist) ok it works
                } else {//on the 'left'
                    data.is_correct_otherlist = data.word_response_extract == data.word_right_i; //changed in v10
                }
    ...
    * ...
    ...
    var prompt_list  = {
        ...
        stimulus: function(){
            ...
            console.log(listnum)
            if (condi !== "r"){
                return (`Tests of words from <strong>LIST ${listnum}</strong>`)
            }
        },
        choices: "NO_KEYS",
    ...
    var v_finaltest = {
        ...
        on_start: function(trial){
            ...
            if (is_showcorrect_inlog) console.log("Correct answer:",trial.data.wordchosen_finaltest,trial.data.word_right_i,"trial:",JT('testpos_final'));
        ...
        questions: function(){
            ...
            listnum = JT('listnumber_1to10');
            wc = JT('wordcondi');
            pw = JT('pairs_which_1or2');
            if (condi!=='r'){//all conditions except random
                if (wc==="repeat"){
                    if (pw===1) listcomp=listnum+1;
                    else if (pw===2) listcomp=listnum-1;
                    return([{
                        prompt: "".concat(` <p style="line-height: 1.5;">Type a word you studied starting with <strong>" ${ftlnow} </strong> ________"</p><p style="line-height: 1.5;"> Words starting with these letters were from <strong>list ${listnum}</strong> and <strong>${listcomp}</strong>. Try to type the word that was from <strong>list ${listnum}</strong>. <br> But if you are not sure, type either word you remembered beginning with those letters. </p>`), //changed!!
                        placeholder:``.concat(`type the word starting with '`,JT('ftl'),`' or 'none'`),
                        required: true
                        }])
                } else if (wc==="unique"){
                    ...
                    return([{
                        prompt: "".concat(` <p style="line-height: 1.5;">Type a word you studied starting with <strong>" ${ftlnow} </strong> ________"</p><p style="line-height: 1.5;"> This word was studied on <strong>list ${listnum}</strong></p>`), //changed!!
                        placeholder:``.concat(`type the word starting with '`,JT('ftl'),`' or 'none'`),
                        required: true
                        }])
                }
            } else{//random conidtion
                return([{
                    prompt: "".concat(` <p style="line-height: 1.5;">Type a word you studied starting with <strong>" ${ftlnow} </strong> ________"`), //changed!!
                    placeholder:``.concat(`type the word starting with '`,JT('ftl'),`' or 'none'`),
                    required: true
                    }])
            }
        ...
        on_finish: function(data){
            ...
            // correct currentlist different for pair 1 or 2
            if (data.pairs_which_1or2===1){//first in pair
                data.is_correct_currentlist = data.word_response_extract === data.word_left_i;//correct for unique and corr list in repeat
                data.is_correct_otherlist = data.word_response_extract === data.word_right_i;//Note: other list correct even when in unique condition, though that correct is meaningless, or that correct is pure of chance but not memory
            } else if (data.pairs_which_1or2===2){
                data.is_correct_currentlist = data.word_response_extract === data.word_right_i;//correct for unique and corr list in repeat
                data.is_correct_otherlist = data.word_response_extract === data.word_left_i;
            }

            //is_correct_any records correct for repeat or unique condition, repeat condition correct when word from either is correct
            if (data.wordcondi==="repeat"){
                data.is_correct_any = data.is_correct_currentlist || data.is_correct_otherlist;
            }else if (data.wordcondi === "unique"){
                data.is_correct_any = data.is_correct_currentlist;
            }

            //location by word left or right regardless of which pair of list
            if (data.word_response_extract === data.word_left_i){
                data.correct_location = 1;
            }
            else if (data.word_response_extract === data.word_right_i && data.wordcondi === "repeat" ){
                data.correct_location = 2;
            }
            else {data.correct_location = 0}

            // console.log(data.correct_location,data.is_correct_any,data.is_correct_currentlist)
        },//end of on_finish
        // post_trial_gap: posgap_duration
    };
    ...
    var finaltest_feedback = {
        ...
        stimulus: function(){
            ...
            lasttrial_obj = jsPsych.data.get().last(1).values()[0];
            listnum = lasttrial_obj.listnumber_1to10;
            listnum1 = lasttrial_obj.listnumber_1to10_inpair1 ? lasttrial_obj.listnumber_1to10_inpair1 : listnum;
            listnum2 = lasttrial_obj.listnumber_1to10_inpair2;
            ...
            if (lasttrial_obj.is_correct_any) {correct_text = "<strong></strong> "}
            else {correct_text="The word you typed <strong>WAS NOT STUDIED!</strong> "}
            // correct_text = ""
            
            // listcorrect = lasttrial_obj.wordcondi==="repeat" && lasttrial_obj.is_correct_otherlist ? `The word you typed is from List ${listnum}` : "";
            if (lasttrial_obj.correct_location===1) {listcorrect = `The word you typed <strong> WAS FROM LIST ${listnum1}</strong>`} 
            else if (lasttrial_obj.correct_location===2) {listcorrect= `The word you typed <strong> WAS FROM LIST ${listnum2}</strong>`}
            else if (lasttrial_obj.correct_location===0) {listcorrect="<br>"} 

            // if (lasttrial_obj.is_correct_otherlist && lasttrial_obj.wordcondi==="repeat") addtext = "<strong>The "; 
            // else if (lasttrial_obj.correct_location!==0) addtext = "The ";
            // else addtext = "";
            if (condi==="r") {
                correct_text = lasttrial_obj.is_correct_any ? `The word you typed <strong>were STUDIED</strong>` :  `The word you typed <strong>were NOT STUDIED</strong>`;
                listcorrect = "" 
            }

            if (lasttrial_obj.is_notremembered) return(`.`)
            
            // console.log(listnum1, listnum2, lasttrial_obj.correct_location,listcorrect)
            return ``.concat(feed_note0,correct_text,listcorrect)
        },
        data:{
    ...
    var toolUsageTrial = {
        ...
        stimulus: 'You now finished all parts of experiment. <br> Did you use any tools to help you remember? <br><br> Please provide an honest answer.<br> This answer will not affect your reward.',
        choices: ['Yes', 'No'],
    ...
    var surveyend = {
        type: jsPsychSurveyText,
        questions: [
            { prompt: `Did you find the second part of test (instrucitons and feedback) understandable? <br>Did you have enough time to read the feedback of each trial in the second part of the test?<br>Type in your feedback below if you there is any confusion or if you didn't find the feedback presentation time to be enough. If you find all parts understandable, press 'enter' to skip`, name: 'surveyend' }
        ],
    ...
    var final_instruction = {
        on_start: function(trial){
            window.onbeforeunload = null;
            firstcr = jsPsych.data.get().select('correct').values;
            secondcr = jsPsych.data.get().select('is_correct_any').values;
            ...
            jsPsych.data.addProperties({
                all_accumulated_accuracy: average([...firstcr,...secondcr])
            });
        },
        type: jsPsychHtmlKeyboardResponse,
        stimulus: function(){
            firstcr = jsPsych.data.get().select('correct').values;
            secondcr = jsPsych.data.get().select('is_correct_any').values;
            nownow =  average([...firstcr,...secondcr]);
            console.log(nownow)
    ...
    timeline.push(toolUsageTrial,penPaperTrial,surveyend)
