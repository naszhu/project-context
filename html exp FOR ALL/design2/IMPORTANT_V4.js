 ...
 * Date: 2023-12-23
 *
 * Description: codeversion 4
 * changed how accumulated accuracy is calculated at end! add averaged accuracy and so on
 *calculation of previous codeversion is incorrect
 *  ...
 ****************************************************************************************************************/
...
//****************************************************
    ...
    * ...
    ...
    on_start: function(trial){
        window.onbeforeunload = null;
        c1 = jsPsych.data.get().select('is_correct_avg_repeat_precise').values;
        c2 = jsPsych.data.get().select('is_correct_unique_precise').values;
        c3 = jsPsych.data.get().select('correct').values;
        // nownow = jsPsych.data.get().select(['word1_response','word2_response','correct']);
        // console.log(nownow)
        // console.log([...firstcr,...secondcr,...initialcr])
        // console.log(average([...firstcr,...secondcr,...initialcr]))
        jsPsych.data.addProperties({
            all_accumulated_accuracy: parseFloat(average([...c1,...c2,...c3]).toFixed(4))
        });
    },
    type: jsPsychHtmlKeyboardResponse,
    stimulus: function(){
