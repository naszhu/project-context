#= ===========================================================================================================
   ...
   Notes       : this is only to compare with V4
   =========================================================================================================== =#
...
n_units_time_restore = n_units_time -2
...
const p_listchange_finalstudy = 0.2; #either of these contribute to differences between list 
...
function calculate_two_step_likelihoods(probe::EpisodicImage, image_pool::Vector{EpisodicImage}, p::Float64)::Tuple{Vector{Float64}, Vector{Float64}} 
    ...
    for ii in eachindex(image_pool) 
        ...
        if is_firststage
            # println(context_likelihood,context_tau)
...
function probe_evaluation(image_pool::Vector{EpisodicImage}, probes::Vector{Probe})::Array{Any}
    ...
    for i in eachindex(probes)
        ...
        ctx, likelihood_ratios = calculate_two_step_likelihoods(probes[i].image, image_pool, 1.0); #proportion is all
        # println(ctx)
        ...
        results[i] = (decision_isold = decision_isold, is_target = probes[i].classification, odds = odds, nactivated = length(likelihood_ratios)/length(image_pool));
