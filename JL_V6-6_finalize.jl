...
I. 
const p_driftAndListChange_finaltest =0 #probe probability change for context
===
...
p_driftAndListChange_finaltest = ones(10) * 0.4 #0.1 prob list change for final test
    ...
    for list_number in lists #lists is [1] for random condition
        ...
        if (icount !=1) && (condition != :true_random)
            for cf in eachindex(listcg)
                if rand() < p_driftAndListChange_finaltest[icount] #cf.change_probability # this equals p_change
