...
------------------
A change from v6-6_3 - explore for E3: why Hs drop across lists but not Foil
1. current context at retrieval becomes less and less perfectly reinstated across lists,
=========================================================================================================== =#
...
criterion_initial = ones(n_probes)*1;#the bigger the later number, more close hits and CR 
...
p_reinstate_rate = LinRange(0.5,0.1,n_lists ) #0.4 #prob of reinstatement
...
println("prob of each all features had reinstate after 3 $(1-(1-p_reinstate_rate[1])^3)")
...
u_star_storeintest = u_star #for word # ratio of this and the next is key for T_nt > T_t
u_star_context=vcat(0.08, ones(n_lists-1)*0.05)

const n_units_time = 13#number of s                           
    ...
    for i in eachindex(probetypes)
        ...
        for ict in eachindex(test_list_context)
            if ict < Int(round(nct * p_reinstate_context)) #stop reinstate after a certain number of features
                ...
                if (test_list_context[ict] != list_change_features[ict]) & (rand() < p_reinstate_rate[list_num])
        ...
        if is_UnchangeCtxDriftAndReinstate
            ...
            for ict in eachindex(test_list_context_unchange)
                if ict < Int(round(nct * p_reinstate_context))
                    ...
                    if (test_list_context_unchange[ict] != general_context_features[ict]) & (rand() < p_reinstate_rate[list_num])
                        # println("here")
