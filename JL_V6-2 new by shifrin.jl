...
const w_context =60; #first half unchange context, second half change context, third half word-change context
ratio_U=0.3 #ratio of general(unchanging) context
...
p_listchange_finaltest = ones(10)*0.0
...
w_word = 30; # number of word features, 30 optimal for inital test, 25 for fianal 
is_firststage = true;
...
criterion_initial =  vcat(LinRange(1, 0.6, 20),ones(n_probes-min(20,n_probes)).-0.4)
...
"""
function generate_probes(studied_words::Vector{Word}, list_change_features::Vector{Int64},test_list_context::Vector{Int64}, general_context_features::Vector{Int64}, list_num::Int64)::Vector{Probe}
    ...
    for i in eachindex(probetypes)
        ...
        current_context_features = vcat(general_context_features, deepcopy(test_list_context)) ; #here needs a deepcopy, otherwise the front remembered context change with later ones  
...
function calculate_two_step_likelihoods(probe::EpisodicImage, image_pool::Vector{EpisodicImage}, p::Float64, iprobe::Int64)::Tuple{Vector{Float64}, Vector{Float64}} 
    ...
    for ii in eachindex(image_pool) 
        ...
        if is_test_allcontext  #here is secon  stage would be wrong
            context_likelihood = calculate_likelihood_ratio(probe_context,image_context,g_context,c )  # .#  Context calculation
        else  #not testing all context but change only; the last 1/2 half are list_change and word_change context
            context_likelihood = calculate_likelihood_ratio(probe_context[nU + 1 : end],image_context[nU + 1 : end],g_context,c )  # .#  Context calculation
        end
...
function probe_evaluation(image_pool::Vector{EpisodicImage}, probes::Vector{Probe},list_change_features::Vector{Int64},general_context_features::Vector{Int64})::Array{Any}
    ...
    for i in eachindex(probes)
        ...
        for j in eachindex(unique_list_numbers)
            ...
            results[n_listimagepool*(i-1)+j] = (decision_isold = decision_isold, is_target = probes[i].classification, odds = odds, ilist_image=j,Nratio_imageinlist = nimages_activated/nimages, N_imageinlist=nimages_activated,Nratio_iprobe = nav, testpos = i, studypos=probes[i].image.word.studypos);
        end
...
function simulate_rem()
    ...
    df_inital = DataFrame(list_number = Int[], test_position = Int[], simulation_number = Int[], decision_isold = Int[], is_target = Bool[],odds = Float64[], Nratio_iprobe = Float64[], Nratio_iimageinlist = Float64[], N_imageinlist=Float64[], ilist_image = Int[], study_position=Int[])
    ...
    for sim_num in 1:n_simulations
        ...
        for list_num in 1:n_lists
            ...
            for _ in 1:n_RI_change[list_num]
            ...
            probes = generate_probes(word_list, list_change_context_features,test_list_context,general_context_features, list_num); #probe number is current list number, get probes of current list 
            # println("list $(list_num), ")
            ...
            for (ires, res) in enumerate(results) #1D array, length is 20 words
                ...
                row = [list_num, res.testpos, sim_num, res.decision_isold, tt, res.odds, res.Nratio_iprobe, res.Nratio_imageinlist, res.N_imageinlist, res.ilist_image, res.studypos] # Add more fields as needed
        ...
        for ccf in eachindex(general_context_features)
            if rand() <  0.2 #cf.change_probability # this equals p_change
